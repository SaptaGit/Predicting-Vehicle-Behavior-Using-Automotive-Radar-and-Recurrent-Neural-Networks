########################################################################################################################
# Orginal File copied from the FilterNetworkV15.py from this folder. For any issues compare with the mentioned file.
########################################################################################################################
# This version is only for data creation with the I-80 or US-101 dataset.
# Two different csv files are used to create sepearte train and test vehicles, so that both TV and SVs can bes added
...
folderName = '/media/sdd/sap/FilterNet/data/I804SurroundingMixed2048V1'  #   I80Surrounding4MoreLaneV1'  I804SurroundingMixed1024V1
...
resultFileName = '/media/sdd/sap/Junction/results/FilterNetVRun20Run24.txt'
...
inputFeatureCount = 8  # 6 -> [localX,localY,velocityX,velocityY,laneID,movement,headwaySpace,headwayTime]
...
# Validation vehiles
trainVehileCount = 1700   # 1200      #650    #2300   #2200  # 2000 # 2200      #2200    #1600 # 2200   totalVehileCount
validationVehicleCount = 350    # 400    # 150   #350    #  380  # 400     # #400  # removed for transfer learning..
        ...
        for eachOtherVechiles in otherVechiles:
            ...
            if(inputOrDecoder == inputStr):
                if((otherDist>maximumSurroundingCarDist)):
                    ...
                    tempInput.extend([otherRelativeX,otherRelativeY,otherVelocityX,otherVelocityY,otherLaneID,otherManeuver,otherCsvHeadwaySpace,otherCsvHeadwayTime])
...
def ProcessByVehicle(processItme):
    ...
    for idx in range(historyTemporal+10+prevNextFrameCount,currentVehicleLength-futureTemporal-10-prevNextFrameCount): 
        ...
        for jdx in range(idx-historyTemporal,idx):
            ...
            # With the headway space and time 
            tempInput = [localX,localY,velocityX,velocityY,laneID,movement,csvHeadwayDistVal,csvHeadwayTimeVal]
...
def GeneretorWriteData(genFolderName,genFileNameList,totalGenArray):
    ...
    # # # # #if(len(totalGenArray[0]) == len(totalGenArray[1]) == len(totalGenArray[2])):
...
def TrainData(inputFileName,totalTrainOrValStr):
    ...
    for eachKey in selectedVehilces:
        ...
        ############################################################################
        # # # # Chnaged for seperate data train/test
    ...
    print('VelocityX min :' + str(minVelocityX))      # Commenting out for vanilla
    print('VelocityX max :' + str(maxVelocityX))      # Commenting out for vanilla
    print('VelocityY min :' + str(minVelocityY))      # Commenting out for vanilla
    print('VelocityY max :' + str(maxVelocityY))      # Commenting out for vanilla
    ...
    print('CSV Headway time min :' + str(maxHeadwayTime))
    ...
    # Since the validation is the last call write the min max at the validation Call
    if(totalTrainOrValStr == validationStr):
    ...
    # Since now there would be two calls for train and val using file mode 'a'
...
def ValidationSetProcess():
    ...
    os.system("taskset -p -c 12,13,14,15,16 %d" % os.getpid()) 
    coreCount = 2  # 17   # Don't change. Decided based on the single GPU memory...
    ...
    # # #     p = mp.Process(target=ValidationSetPredict, args=(totalSampleCount,eachSplittedList))
    # # #     processes.append(p)
    # # #     p.start()
    ...
    # This normal for-loop for local debugging. Just take the first sublist
    ValidationSetPredict(totalSampleCount,['000001'])
    ...
    ##################################################################################
    print('Calculating final error for the poses estimated using true motion!!!')
    ...
    ##################################################################################
    print('Calculating final error for the poses estimated using filter using both predicted motion and pose!!!')
    finalMotionPositionFilterError = np.zeros(futureTemporal)
    # iterate the manager list to calcualte final error
    for eachMotionPostionFilterItem in positionMotionFilterError:
        finalMotionPositionFilterError = finalMotionPositionFilterError + np.array(eachMotionPostionFilterItem)
    
    # Convert to array for variance calculation
    positionMotionFilterArray = np.array(positionMotionFilterError)
    finalPositionMotionFilterVar = np.var(positionMotionFilterArray, axis = 0)

    # Calculate the mean variance of each sample for the filter using both position and motion
    meanVarMotionPositionFilterX = np.zeros(futureTemporal)
    meanVarMotionPositionFilterY = np.zeros(futureTemporal)
    # iterate the manager list to calcualte final error
    for eachPositionMotionFilterVarItem in positionMotionFilterVar:
        meanVarMotionPositionFilterX = meanVarMotionPositionFilterX + np.array(eachPositionMotionFilterVarItem)[:,0]
        meanVarMotionPositionFilterY = meanVarMotionPositionFilterY + np.array(eachPositionMotionFilterVarItem)[:,1]

    ##################################################################################

    ##################################################################################
    print('Calculating final error for the poses estimated using filter using only predicted pose and last true motion!!!')
    finalOnlyPositionFilterError = np.zeros(futureTemporal)
    # iterate the manager list to calcualte final error
    for eachOnlyPostionFilterItem in onlyPositionFilterError:
        finalOnlyPositionFilterError = finalOnlyPositionFilterError + np.array(eachOnlyPostionFilterItem)

    # Convert to array for variance calculation
    onlyPositionFilterArray = np.array(onlyPositionFilterError)
    finalOnlyPositionFilterVar = np.var(onlyPositionFilterArray, axis = 0)

    meanVarOnlyPositionFilterX = np.zeros(futureTemporal)
    meanVarOnlyPositionFilterY = np.zeros(futureTemporal)
    # iterate the manager list to calcualte final error
    for eachOnlyPositionFilterVarItem in onlyPositionFilterVar :
        meanVarOnlyPositionFilterX = meanVarOnlyPositionFilterX + np.array(eachOnlyPositionFilterVarItem)[:,0]
        meanVarOnlyPositionFilterY = meanVarOnlyPositionFilterY + np.array(eachOnlyPositionFilterVarItem)[:,1]
    ##################################################################################

    ##################################################################################
    print('Calculating final error for the poses estimated using filter using both predicted motion and pose, with no particle regenaration!!!')
    finalNonReGenFilterError = np.zeros(futureTemporal)
    # iterate the manager list to calcualte final error
    for eachNonGenFilterItem in nonReGenFilterError:
        finalNonReGenFilterError = finalNonReGenFilterError + np.array(eachNonGenFilterItem)

    # Convert to array for variance calculation
    nonReGenFilterArray = np.array(nonReGenFilterError)
    finalnonReGenFilterVar = np.var(nonReGenFilterArray, axis = 0)

    meanVarNonReGenFilterX = np.zeros(futureTemporal)
    meanVarNonReGenFilterY = np.zeros(futureTemporal)
    # iterate the manager list to calcualte final error
    for eachNonGenFilterItem in nonReGenFilterVar:
        meanVarNonReGenFilterX = meanVarNonReGenFilterX + np.array(eachNonGenFilterItem)[:,0]
        meanVarNonReGenFilterY = meanVarNonReGenFilterY + np.array(eachNonGenFilterItem)[:,1]
    ...
    print('final error for the poses estimated using True motion!!')
    print(finalMotionError/count)
    print('final var for the poses estimated using True motion!!')
    print(finalMotionErrorVar)


    print('Final error for the poses estimated using filter using both predicted motion and pose!!')
    print(finalMotionPositionFilterError/count)
    print('Final var for the poses estimated using filter using both predicted motion and pose!!')
    print(finalPositionMotionFilterVar)
    print('Final mean X var for the poses estimated using filter using both predicted motion and pose!!')
    print(meanVarMotionPositionFilterX/count)
    print('Final mean Y var for the poses estimated using filter using both predicted motion and pose!!')
    print(meanVarMotionPositionFilterY/count)


    print('Final error for the poses estimated using filter using only predicted pose and last true motion!!')
    print(finalOnlyPositionFilterError/count)
    print('Final var for the poses estimated using filter using only predicted pose and last true motion!!')
    print(finalOnlyPositionFilterVar)
    print('Final mean X var for the poses estimated using filter using only predicted pose and last true motion!!')
    print(meanVarOnlyPositionFilterX/count)
    print('Final mean Y var for the poses estimated using filter using only predicted pose and last true motion!!')
    print(meanVarOnlyPositionFilterY/count)


    print('Final error for the poses estimated using filter using both predicted motion and pose with low and constant variance particle regeneration!!')
    print(finalNonReGenFilterError/count)
    print('Final var for the poses estimated using filter using both predicted motion and pose with low and constant variance particle regeneration!!')
    print(finalnonReGenFilterVar)
    print('Final mean X var for the poses estimated using filter using both predicted motion and pose with low and constant variance particle regeneration!!')
    print(meanVarNonReGenFilterX/count)
    print('Final mean Y var for the poses estimated using filter using both predicted motion and pose with low and constant variance particle regeneration!!')
    print(meanVarNonReGenFilterY/count)
    ...
    errorTypeStr = 'Final error for the poses estimated using filter using both predicted motion and pose!!'
    ResultWriteFile(errorTypeStr,finalMotionPositionFilterError/count)
    errorTypeStr = 'Final var for the poses estimated using filter using both predicted motion and pose!!'
    ResultWriteFile(errorTypeStr,finalPositionMotionFilterVar)
    errorTypeStr = 'Final mean X var for the poses estimated using filter using both predicted motion and pose!!'
    ResultWriteFile(errorTypeStr,meanVarMotionPositionFilterX/count)
    errorTypeStr = 'Final mean Y var for the poses estimated using filter using both predicted motion and pose!!'
    ResultWriteFile(errorTypeStr,meanVarMotionPositionFilterY/count)


    errorTypeStr = 'Final error for the poses estimated using filter using only predicted pose and last true motion!!'
    ResultWriteFile(errorTypeStr,finalOnlyPositionFilterError/count)
    errorTypeStr = 'Final var for the poses estimated using filter using only predicted pose and last true motion!!'
    ResultWriteFile(errorTypeStr,finalOnlyPositionFilterVar)
    errorTypeStr = 'Final mean X var for the poses estimated using filter using only predicted pose and last true motion!!'
    ResultWriteFile(errorTypeStr,meanVarOnlyPositionFilterX/count)
    errorTypeStr = 'Final mean Y var for the poses estimated using filter using only predicted pose and last true motion!!'
    ResultWriteFile(errorTypeStr,meanVarOnlyPositionFilterY/count)


    errorTypeStr = 'Final error for the poses estimated using filter using both predicted motion and pose with low and constant variance particle regeneration!!'
    ResultWriteFile(errorTypeStr,finalNonReGenFilterError/count)
    errorTypeStr = 'Final var for the poses estimated using filter using both predicted motion and pose with low and constant variance particle regeneration!!'
    ResultWriteFile(errorTypeStr,finalnonReGenFilterVar)
    errorTypeStr = 'Final mean X var for the poses estimated using filter using both predicted motion and pose with low and constant variance particle regeneration!!'
    ResultWriteFile(errorTypeStr,meanVarNonReGenFilterX/count)
    errorTypeStr = 'Final mean Y var for the poses estimated using filter using both predicted motion and pose with low and constant variance particle regeneration!!'
    ResultWriteFile(errorTypeStr,meanVarNonReGenFilterY/count)
...
def TrainingWorker():
    ...
    class EarlyStoppingByLossVal(callbacks.Callback):
        def __init__(self, monitor='val_loss', value=1.2, verbose=0):
    ...
    os.system("taskset -p -c 9,10,11,12,13,14,15,16 %d" % os.getpid())

    # model.fit(trainGen, epochs=nepochs, verbose=1, validation_data=valGen, callbacks=callbacks_list)
    model.fit(trainGen, steps_per_epoch=stepsPerEpoch, epochs=nepochs, verbose=1, validation_data=valGen, validation_steps=valStepsPerEpoch, callbacks=callbacks_list, max_queue_size=24, workers=12, use_multiprocessing=True)
...
if __name__ == '__main__':
    ...
    if(processOrRead == processStr):
        ...
        os.mkdir(folderName)
        # Prepare the data
        TrainData(trainTrajFilePath, trainStr)
        TrainData(testTrajFilePath, validationStr)
        ...
        # # # # # Changing to normal function to see speed
        # # # TrainingWorker()

        # # # Changing to Pool to see speed
        # # numberofCores = 20
        # # pool = mp.Pool(numberofCores)
        # # pool.map(TrainingWorker)
