###################################################################################################################
# The Retrain technique with I-80 data and new maneuver class. on sen server
# Using diff step decay, in case very bad, change it back.. -> very bad, file names -> resultFileName = 'ModifiedDecoderV10Run21.txt' historyFileName = 'ModifiedDecoderV10LossRun21.txt'
# Using the old step decay and epochs.. -> batch size 2048 is still preety bad. Retrying with lower batch size 
# Also changed the second step decay as the batch size is low (256) 
# Do the first round training only with 2048 and the retrains with 256
# First round training -> running... (for retrain keep the same model and result files...)
# Using the same trained model (with 2048) but only retrain with lower batch size 256 -> still no good..
# Result files resultFileName = 'ModifiedDecoderV10Run25.txt' historyFileName = 'ModifiedDecoderV10LossRun25.txt'
# In case still not good use the 1024 data and start from the beggining.....
# Starting from the beggining with batch 1024 and same step decays -> still no good, close to 19.7..
# Starting from current stage with batch 128 and changed step decay -> no good close to 60.. (same result files v26)
# Result Files resultFileName = 'ModifiedDecoderV10Run26.txt' historyFileName = 'ModifiedDecoderV10LossRun26.txt'
# If still no good use the old architecture 
# Using the old architecture -> running.. (models backed up at 4.86 val error under models folder. always start from there)
...
# jump 4 instead of 15, second step 8.0 insted of 2.0 and 0.001 instead of 0.002, not reloading the weights -> no good, close to 9
# resultFileName = 'ModifiedDecoderV10Run29.txt' historyFileName = 'ModifiedDecoderV10LossRun29.txt'
# jump 4 instead of 15, second step 20.0/0.002 (back to old setting) not reloading the weights, increased second epoch count -> running.. (check the ram conspumption -> good)
# Check the loss log file to double check the epoch count (180) -> epocuh count checked and correct
...
# resultFileName = 'ModifiedDecoderV10Run34.txt'      historyFileName = 'ModifiedDecoderV10LossRun34.txt'
# reload the weights -> slight overfitting .. (change the jump from 4 to 2)...
# Result files resultFileName = 'ModifiedDecoderV10Run35.txt'  historyFileName = 'ModifiedDecoderV10LossRun35.txt'
# Reloade the weights and jump is 1 -> running...
...
folderName ='/media/disk5/sap/FilterNet/data/I804SurroundingMoreLane1024V1'   # I804SurroundingMoreLane2048V1'
...
# folderName = '/media/sdd/sap/FilterNet/data/I804SurroundingMoreLane1024V1'    # EligibilityCheckRemovedD1   smallTest   
...
# Path for Sen server folder
encoderModelFilename = '/media/disk1/sap/Junction/ModV11EncoderRun11.h5'
decoderModelFilename = '/media/disk1/sap/Junction/ModV11DecoderRun11.h5'
mainModelFileName  = '/media/disk1/sap/Junction/ModV11MainModelRun11.h5'
# Path for Big Screen Server folder
# encoderModelFilename = '/home/sap/Sap/Junction/ModV10Encoder6.h5'
# decoderModelFilename = '/home/sap/Sap/Junction/ModV10Decoder6.h5'
# mainModelFileName  = '/home/sap/Sap/Junction/ModV10MainModel6.h5'


# Specify the result file to store each sample error
resultFileName = 'ModifiedDecoderV10Run36.txt'
historyFileName = 'ModifiedDecoderV10LossRun36.txt'
...
batchSize = 1024   # 2048      #1024   #256  128
initialNumberEpochs = 40  # 40   #40  #  40  #40   #40   #  30   #   30  #30
secondNumberEpochs = 180   # 60  # 80
...
def elu_plus_one_plus_epsilon(x):
    from keras import backend as K
    from keras.layers import ELU, Add
...
def second_step_decay(epoch):
    initial_lrate = 0.002     # 0.002
    drop = 0.5
    epochs_drop = 20.0   # 20.0
    lrate = initial_lrate * math.pow(drop, math.floor((1+epoch)/epochs_drop))
    if lrate < 0.00001:
        lrate = 0.00001
    return lrate
...
def ModelArch():
    ...
    from keras.layers import  Input,LSTM,Dense,Concatenate, subtract, Lambda, LeakyReLU, ELU, BatchNormalization, Dropout, Add
...
def IntermediatePredictionForTraining(processItemSubList,sublistAssignedGpu):
   ...
   # Saniy check Min max values should be same as -9999 or 99999 after update
    ...
    for eachProcessItem in processItemSubList:
        ...
        for idx in range(historyTemporal+6,currentReleventVehicleLength-futureTemporal-6):   #  15
...
def _training_worker_gen():
    ...
    tf.logging.set_verbosity(tf.logging.ERROR)   #compat.v1.
...
def _training_worker(XTrain,decoderTrainInput,YClassTrain,YVelTrain,YPoseTrain,NumberEpochs,XVal,decoderValInput,YClassVal,YVelVal,YPoseVal):
    ...
    # create the final callback and learning schedulers
