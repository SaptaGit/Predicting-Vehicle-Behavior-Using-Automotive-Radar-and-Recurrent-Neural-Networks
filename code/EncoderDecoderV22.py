...
# Adam(),  BatchNorm,  K.set_learning_phase for batchNorm, QuickData(small new data), batch -> 128, epochs_drop = 6.0, (File 18 Will have same but batch -> 256) early 0.17
# old data, no batchNorm, early 0.15, batch 1024, epochs_drop = 10.0, RMSProp(), ELU() --> not fitted error 4.5 around 
# old data, no batchNorm, early 0.18, batch 2048, epochs_drop = 6.0, Nadam(), ELU() --> 4.74
# new data, no batchNorm, early 0.18, batch 2048, epochs_drop = 10.0, Nadam(), ELU() and confusion --> 5.1 not fitted
# new data, no batchNorm, early 0.21, batch 2048, epochs_drop = 6.0, Nadam(), ELU() initial_rate = 0.002 and confusion --> 4.9
# new data, no batchNorm, early 0.24, batch 2048, epochs_drop = 6.0, Nadam(), ELU() initial_rate = 0.002 and confusion --> 5.1
# new data, no batchNorm, early 0.22, batch 2048, epochs_drop = 5.0, Nadam(), ELU() initial_rate = 0.002 and confusion --> 5.2 (not fitted)
# new data, no batchNorm, early 0.21, batch 2048, epochs_drop = 7.0, Nadam(), ELU() initial_rate = 0.002 and confusion --> 5.3
# new data, no batchNorm, early 0.18, batch 2048, epochs_drop = 7.0, Nadam(), ELU() initial_rate = 0.002 and confusion --> 5.5
# new data, no batchNorm, early 0.20, batch 2048, epochs_drop = 5.0, Nadam(), ELU() initial_rate = 0.002 and confusion --> 4.8 (not fitted)
# new data, no batchNorm, early 0.20, batch 2048, epochs_drop = 4.0, Nadam(), ELU() initial_rate = 0.002 and confusion --> 4.8
# new data, no batchNorm, early 0.18, batch  256, epochs_drop = 2.0, Nadam(), ELU() initial_rate = 0.002 and confusion --> 5.6
# old data, no batchNorm, early 0.10, batch  256, epochs_drop = 2.0, Nadam(), ELU() initial_rate = 0.002 and confusion --> 5.8
# old data, no batchNorm, early 0.15, batch  128, epochs_drop = 2.0, RMSprop(), leakyRelu() initial_rate = 0.001 and confusion --> running leaky 0.3  6.6  65k 6.47
# early 0.25, 256, leaky 0.5 -> 5.8...repeat->5.3
# all same early 0.22->8.5...repeat->6.8
# leaky 0.5 early 0.22 batch 256 step 2.0, lrate 0.000001 (one extra zero)->5.6...repeat->5.3...
# leaky 0.5 early 0.18 batch 256 step 2.0, lrate 0.000001 (one extra zero)-> 5.1...repeat->5.2...
# leaky 0.1 early 0.18 batch 256 step 2.0, inital_lrate 0.0001, lrate 0.000001 (one extra zero)-> 5.2 around...
# leaky 0.3, batch 2048 early 0.22 step 5.0 initial_lrate 0.002, lrate 0.00001, Nadam, motion, shuffle -> 
# early 1.1 -> 4.1
# early 1.0, leaky 1.0 -> 4.32...
# early 1.1, 5 surrounding running...
# Sen server 3rd terminal
import os
os.environ["CUDA_VISIBLE_DEVICES"]="5"  #5
...
# Path for local folder
# folderName = '/home/saptarshi/PythonCode/Junction/Surrounding5RelativeDecoderJuncEligibleCheck'     # old one without check ..... Surrounding4RelativeDecoderJuncDist
...
# Path for Sen server folder
encoderModelFilename = '/media/disk1/sap/Junction/OldDataEncoder22V1.h5'
decoderModelFilename = '/media/disk1/sap/Junction/OldDataDecoder22V1.h5'
# Path for Big Screen Server folder
...
resultFileName = 'V22ModidiedDecoder101.txt'
...
leakyAlphaValue = 1.0  # (0.3 -> Leaky ReLU)
...
# Validation vehiles
totalVehileCount = 1000 #1600 # 2200
validationVehicleCount = 150 # #400
