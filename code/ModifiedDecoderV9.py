...
# Added BatchNorm layer only 4-3-2 at each stage. just to see if it is fixing the obvious overfitting issue. -> Not done, error..
# Let it run all the way end just to see.... BatchNorm may make things worse.. but worth try....
# BatchNormalization is still showing the same error, Trying with DropOut layer 2 at each stage
# Probably Dropout is better than BatchNorm for RNNs,
# BathcNorm is fixed in this version using tensorflow-gpu version = 2.0.0
# There is still a warning regarding optimizer and may need fixing if results are too bad
# Currently the training=True flag is not set for DropOut (use in both training and testing), Can be done in next round...
##########################################################################################################################
# BathcNorm is fixed in this version using tensorflow-gpu version = 1.4.1
# If still bad try the moving average and var for batch norm train/inference technique
##########################################################################################################################
# Try to split all vehicles ids first, asign cores/gpus to use continuously, instead of reloading again and again per vehicle. 
# for this reason GPUS and cores are emptly most of the time.... 
# same way the samples are being tested in the sen server codes..  done...
##########################################################################################################################
############################################# RUN RESULTS AFTER BatchNorm ################################################
# Loss and error dump file names 
# resultFileName = 'ModifiedDecoderV9Run20.txt'
# historyFileName = 'ModifiedDecoderV9LossRun20.txt'
# Error and loss values are not going down netiher for train or val samples.
# The overffiting problem (difference between train and val loss fixed slightly but still not good enough)
# Worth adding few more batch norms and using the kind of "trainig=True" flag for batchnorm layer so that 
# it will use the moving average, which probably better than normal batchnorm
##########################################################################################################################
# Adding few more batch norms and training=True kind of flag for the batch norm layer which will apperanlty use the 
# leanred mean and var while normalizing each batch during testing. As during testing there is only one sample in the batch and
# not the same sized batch used during testing 
# Added few more batchnorm layers, for the training-testing discrepency used K.set_learning_phase (0 = test, 1 = train) flag
# at the moment, may need more analysis. 
##########################################################################################################################
# Still nothing good, currently trying the L2 regularizer technique. Simpling adding regulaizer argument in the dense layers
# currently running..
# Still not good enough, Changes the regularzer values to 0.1 from 0.01 -> running..
# changed from return to continue, check both train and val list and restart with training..
##########################################################################################################################
...
encoderModelFilename = '/home/sap/Sap/Junction/ModV9Encoder15.h5'
decoderModelFilename = '/home/sap/Sap/Junction/ModV9Decoder15.h5'
mainModelFileName  = '/home/sap/Sap/Junction/ModV9MainModel15.h5'


# Specify the result file to store each sample error last one (ModifiedDecoderV5Run15.txt, ModifiedDecoderV5LossRun1.txt)
resultFileName = 'ModifiedDecoderV9Run26.txt'
historyFileName = 'ModifiedDecoderV9LossRun26.txt'
...
def ModelArch():
    ...
    decoder_dense10a = Dense(1024, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense10 = Dense(512, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense11 = Dense(256, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense12 = Dense(128, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense13 = Dense(64, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense20a = Dense(1024, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense20 = Dense(512, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense21 = Dense(256, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense22 = Dense(128, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense23 = Dense(64, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense30b = Dense(2048, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense30a = Dense(1024, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense30 = Dense(512, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense31 = Dense(256, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense32 = Dense(128, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_dense33 = Dense(64, kernel_regularizer=l2(regularizerValue), bias_regularizer=l2(regularizerValue))
    ...
    decoder_state_input_h1 = Input(shape=(n_units,), name='check_input')
    decoder_state_input_c1 = Input(shape=(n_units,), name='check_input_next')
...
def IntermediatePredictionForTraining(processItemSubList,sublistAssignedGpu):
    ...
    for eachProcessItem in processItemSubList:
        ...
        for idx in range(historyTemporal,currentReleventVehicleLength-futureTemporal,5):
