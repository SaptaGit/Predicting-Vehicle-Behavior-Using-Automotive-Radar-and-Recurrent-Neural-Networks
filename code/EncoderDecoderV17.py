# leaky 0.5, early 0.17 uncheck data batch 128 (6.03) (256 not good!!!) changed lr rate  epochs_drop = 10.0 instead of 5.0 insted of 2.0
# Adam(), Different type BatchNorm, K.set_learning_phase for batchNorm, QuickData(small new data), batch -> 128, epochs_drop = 6.0,  early 0.17
# old data, No BatchNorm, early 0.13, batch 1024 (super high), epochs_drop = 10.0, Adam() --> fitted within 70 epoch and Error 5.46
# old data, No BatchNorm, early 0.13, batch 1024 (super high), epochs_drop = 10.0, Adam() ELU() --> fitted within 50 epoch and Error 6.5
# new data, No BatchNorm , early 0.18, batch 2048 (super high), epochs_drop = 10.0, Nadam() ELU()  Nadam intial_lrate = 0.001 and comfusion matrix--> 5.1
# new data, No BatchNorm , early 0.16, batch 2048 (super high), epochs_drop = 15.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 5.5
# new data, No BatchNorm , early 0.22, batch 2048 (super high), epochs_drop = 8.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 4.8
# new data, No BatchNorm , early 0.21, batch 2048 (super high), epochs_drop = 6.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 4.8
# new data, No BatchNorm , early 0.24, batch 2048 (super high), epochs_drop = 5.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 5.23
# new data, No BatchNorm , early 0.22, batch 2048 (super high), epochs_drop = 6.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 4.8
# new data, No BatchNorm , early 0.20, batch 2048 (super high), epochs_drop = 7.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 5.1
# new data, No BatchNorm , early 0.22, batch 2048 (super high), epochs_drop = 7.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 5.1
# new data, No BatchNorm , early 0.21, batch 2048 (super high), epochs_drop = 5.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 4.9
# new data, No BatchNorm , early 0.20, batch 2048 (super high), epochs_drop = 4.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 4.8
# new data, No BatchNorm , early 0.18, batch  256 (super high), epochs_drop = 2.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 6.6
# new data, No BatchNorm , early 0.18, batch 2048 (super high), epochs_drop = 2.0, Nadam() ELU()  Nadam intial_lrate = 0.002 and comfusion matrix--> 5.1
# new data, No BatchNorm , early 0.15, batch  256 (super high), epochs_drop = 2.0, RMSprop() ELU()  RMSprop intial_lrate = 0.001 and comfusion matrix--> around 5.5 - 6 leaky 0.3
# new data, No BatchNorm , early 0.15, batch  256 (super high), epochs_drop = 2.0, RMSprop() Leaky()  RMSprop intial_lrate = 0.001 and comfusion matrix--> 9.8 leaky 0.3 no converge loss 0.27
# all same early 0.22 (last time 0.27 )
# leaky 0.5, 0.001 intitial, 256, early 0.25 (converged)  .....  3750->4.95.....5.5k->5.0...10k->6...11.5k->6.6...24k->7.3
# all same early 0.25 leaky 0.3 converged->5.6 repeat->9.7
# all same early 0.25 leaky 0.5->6.1...repeat->5.8...repeat->nan...repeat->6.3
# leaky 0.3 early 0.22 batch 256 step 2.0, lrate 0.000001 (one extra zero)->5.35...repeat->6.2...
# leaky 0.5 early 0.18 batch 256 step 2.0, lrate 0.000001 (one extra zero)-> 5.4...repeat->5.45...
# leaky 0.01 early 0.18 batch 256 step 2.0, lrate 0.000001 (one extra zero)-> 5.4
# leaky 0.1 early 0.18 batch 256 step 2.0, inital_lrate 0.0001, lrate 0.000001 (one extra zero)-> around 6.3 for 20K...
# leaky 0.1, batch 2048 early 0.22 step 5.0 initial_lrate 0.002, lrate 0.00001, Nadam, motion, shuffle -> 
# 4.2, 4.5, repeat test ->
# early 1.0 -> 3.95...repeat...3.98... 
# early 1.1 -> 4.0...
# early 1.0-> 4.39
# early 1.1, 5 surroudining running....
# Sen server 1st terminal
#################################################################################
# Think like vehicle in section 1,2,3 or junction 1,2,3 in training 
# and vehicle in section 4,5 and junction 4 for validation 
# before adding the sample to training or validation check the section or intersection id of that sample and decide accrodingly
# make sure to create a dff file.......
#####################################################################################
# for multi taksing the just comment the other parts of the netwrok
# need to change the ValidationSetPredict function as no more manuever or motion to concat...
# Think it through......
#####################################################################################
...
# Path for local folder
# folderName = '/home/saptarshi/PythonCode/Junction/ModifyDecoderCheck'       #   old one without check ..... Surrounding4RelativeDecoderJuncDist
# Path for Sen Server folder
folderName ='/media/disk1/sap/Junction/Surrounding5RelativeDecoderJuncDist'        #   Surrounding4RelativeDecoderJuncDist
...
# folderName = '/home/sap/Junction/Surrounding4RelativeDecoderJuncDistEligibilityCheckSmall'
...
# Path for Sen server folder
encoderModelFilename = '/media/disk1/sap/Junction/OldDataEncoder17V1.h5'
decoderModelFilename = '/media/disk1/sap/Junction/OldDataDecoder17V1.h5'
# Path for Big Screen Server folder
...
resultFileName = 'V17ModidiedDecoder117.txt'
...
# Validation vehiles
totalVehileCount = 1600 # 2200
validationVehicleCount = 200  #400
    ...
    for pdx,eachXVal in enumerate(XVal):
        ...
        for t in range(futureTemporal):
            ...
            if(t<(futureTemporal-1)):
                ...
                targetDecoder = [normalizedPredPoseX,normalizedPredPoseYMotion,normalizedPredVelocity,classPred[0][0][0],classPred[0][0][1],classPred[0][0][2]]
                # Quick check**************************************** t+1 -> t
        ...
        finalMotionError = finalMotionError + np.array(currentMotionError)

        # Write the individual error to text file.
        f.write("%s\n" % currentMotionError)

        # Print in the same line 
        printListNormal = np.around(np.array([finalError[0],finalError[4],finalError[9],finalError[14],finalError[19],finalError[24],finalError[29],finalError[34],finalError[39],finalError[44],finalError[49]])/count, 2)
        printListMotion = np.around(np.array([finalMotionError[0],finalMotionError[4],finalMotionError[9],finalMotionError[14],finalMotionError[19],finalMotionError[24],finalMotionError[29],finalMotionError[34],finalMotionError[39],finalMotionError[44],finalMotionError[49]])/count, 2)
        finalTempError = np.concatenate((printListNormal,printListMotion))
        print('Current Frame : ' + str(pdx) + '/' + str(validationSampleLenght) + ' ', end=' ')
        print(*finalTempError, end='\r', flush=True)
    
    print('All the validation vehicles are predicted...')

    print('Final Validation Distance Error Normal')
    print(finalError/count)
    print('Final Validation Distance Error Motion')
...
class EarlyStoppingByLossVal(callbacks.Callback):
    def __init__(self, monitor='val_loss', value=1.1, verbose=0):
