# no BatchNorm, batch 1024, Nadam, epochs_drop = 5.0 and 10.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 7.3, 24.1, 6.2, 13.6
# no BatchNorm, batch 1024, Nadam, epochs_drop = 5.0 and 15.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 6.7,  9.9, 5.4, 24.3
# no BatchNorm, batch 1024, Nadam, epochs_drop = 7.0 and 20.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 7.7, 17.3, 4.9, 13.7
# no BatchNorm, batch 1024, Nadam, epochs_drop = 7.0 and 25.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 8.8, 11.5, 7.5, 14.8
# no BatchNorm, batch 1024, Nadam, epochs_drop = 7.0 and 22.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 8.1, 16.0, 8.7, 20.8
# no BatchNorm, batch 1024, Nadam, epochs_drop = 5.0 and 20.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 6.8, 9.9 , 7.7
# no BatchNorm, batch 1024, Nadam, epochs_drop = 3.0 and 15.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 6.4, 20.6, 5.0
# no BatchNorm, batch 1024, Nadam, epochs_drop = 3.0 and 10.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 6.7, 17.3, 18.7
# no BatchNorm, batch 1024, Nadam, epochs_drop = 2.0 and 15.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 7.6, 15.0, 10.6
# no BatchNorm, batch 1024, Nadam, epochs_drop = 3.0 and 15.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 7.1, 9.8, 11.7
# no BatchNorm, batch 1024, Nadam, epochs_drop = 7.0 and 20.0, ELU, early 0.20 , intial lrate 0.001, 0.002 -> 8.9,14.5, 10.5 repeat->nan...repeat->8.6, 13.5, 7.1...repeat->8.4, 17.4, 5.2...repeat-> 7.2, 15, 6
# batch 256, step 5.0,15.0->12.7,17.9,9.8...repeat->15.6,13.3,5.1...repeat->13.2,10.4,7.5...repeat->20.5,14.7,8.2
# batch 256 step 5.0,15.0 with extra zero for lrate 0.000001->11.5,12.2,9.7...repeat->11.07,13.1,7.6
# batch 256 step 5.0,15.0 with extra zero for lrate 0.000001 ELU alpha 0.3->11.09,16.8,5.9
# batch 1024 step 7.0,20.0 with extra zero for lrate 0.00001 ELU alpha 0.3-> 8.3,18.9,4.9...repeat->8.0, 19.8,5.0...
# all same ELU alha 0.1->8.3,18.8,5.1
# shuffle 7.8, 21.5,4.78, 
# 9.5, 19, 4.9
# batch 1024, step as v18 epochs reduced... 9.6,18.6,9.7.
# batch 1024 step 7.0,20.0, 0.001, 0.002, ELU alpha 0.1, 5 surrouding-> 7.5,19.5,22.5, repeat...error...
# batch 1024 step 7.0,20.0, 0.001, 0.002, ELU alpha 0.1, 4 surrouding class and vel layer removed (each one)->7.53,10.67,13.08
# batch 1024 step 7.0,20.0, 0.001, 0.002, ELU alpha 0.1, 4 surrouding (added back layers) early 0.20 -> 8.08,23.92,5.72....repeat..8.8,20.6,..6.6.
############################################################################################################################################
######### try removing 'allow_groth_gpu_option' from trainingWorker and see if it can accomodate batch 2048.... now... tried not good!!!!
#############################################################################################################################################
# batch 1024 step 2.0,2.0, 0.002, 0.002, 0.000001, 0.000001, ELU alpha 0.1, 4 surrouding, early 1.0, epochs 40-60 -> 12.3,21.4,6.7....
# batch 1024 step 2.0,2.0, 0.0002, 0.0002, 0.000001, 0.000001, ELU alpha 0.1, 4 surrouding, early 1.2, epochs 40-80 -> 6.7,26.0,8.3...repeat...6.9,29.0,10.0
# batch 1024 step 7.0,20.0, 0.001, 0.002, 0.000001, 0.000001, ELU alpha 0.1, 4 surrouding, early 0.2, epochs 100-120 -> 7.82,16.04,8.7.
# batch 1024 step 7.0,20.0, 0.001, 0.002, 0.00001, 0.00001, ELU alpha 0.1, 4 surrouding, early 0.2, epochs 100-120 -> 8.2,17.9,5.34...repeat...8.12,18.2,5.43...
# batch 1024 step 7.0,20.0, 0.001, 0.002, 0.0001, 0.0001, ELU alpha 0.1, 4 surrouding, early 0.2, epochs 100-120 -> 11.2,18.2,16.5...repeat....9.68,21.8,4.74...repeat.....24.72,13.48.7...repeat...runnning....(initial epoch 20)
# Same thing as above -> 20,16,6 repeat running....26,27,4.87 ... repeat..14,22,5.2...
# batch 1024 step 7.0,20.0, 0.001, 0.002, 0.00001, 0.00001, ELU alpha 0.1, 4 surrouding, early 0.2, epochs 20-60 -> 19,24,19...repeat..running...
# sample count 2102 to keep an eye...
# More re-training loops.... 5
In IntermediatePredictionForTraining function chnage the jump to a litle less, say 10 frames.....
...
from keras.layers import  Input,LSTM, Dense, TimeDistributed, Conv2D, MaxPooling2D, Flatten, Dropout, MaxPooling1D, Concatenate, division, subtract, Lambda, BatchNormalization, LeakyReLU, ELU
import matplotlib.pyplot as plt
...
folderName = '/home/sap/Sap/Junction/EligibilityCheckRemovedD1'
...
encoderModelFilename = '/home/sap/Sap/Junction/Encoder46.h5'
decoderModelFilename = '/home/sap/Sap/Junction/Decoder46.h5'


# Specify the result file to store each sample error last one (ModifiedDecoderV5Run15.txt, ModifiedDecoderV5LossRun1.txt)
resultFileName = 'ModifiedDecoderV5Run119.txt'
historyFileName = 'ModifiedDecoderV5LossRun104.txt'
...
# Model parametrs 
numberOfTrainingLoop = 5 
batchSize = 1024   #256  128
initialNumberEpochs = 20
secondNumberEpochs = 60
...
def IntermediatePredictionProcess(vehicleFileName,trainOrValStr):
    ...
    # Clean up the train and validation process list
    ...
    # Multiprocessing pool technique Call the function
    # pool.map(IntermediatePredictionForTraining,vehicleProcessList)

    # Multiprocessing different technique 
    # processes = []

    # n is the number of item in each sublist decided based on the GPU memory
    n = 50

    # just for quick check shorten the list or pick first 20 item
    # vehicleProcessListSmallCheck = vehicleProcessList[:20]
    ...
    for eachSplitedList in splittedList:
        ...
        # # Kill all the current n process to finish. 
        # for process in processes:
        #     process.close()
