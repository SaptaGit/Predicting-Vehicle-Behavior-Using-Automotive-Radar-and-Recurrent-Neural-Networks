# ELU, early 0.17 uncheck old data batch 128 diff lrate value epochs_drop = 10.0 instead of 5.0 instead of 2.0 
# removed batchNorm as batchNorm is major bad news
# Adam(),  BatchNorm,  K.set_learning_phase for batchNorm, Surrounding4RelativeDecoderJuncDist(good old data), batch -> 128, epochs_drop = 6.0, (same as file 22/18 but with old good data) early 0.17
# old data, New arch (swapped leaky and batch), early 0.17, batch 512 (super high), epochs_drop = 6.0 -- running
# Params matched with the last file ........
# old data, no batchNorm, early 0.23, batch  256, epochs_drop = 2.0, RMSprop(), leakyRelu() initial_rate = 0.001 -> 6.5 (fitted!!! 58/150)
# old data, no batchNorm, early 0.15, batch  256, epochs_drop = 2.0, RMSprop(), leakyRelu() initial_rate = 0.001 -> no converge
# step...2.0, 0.001, leaky 0.3 early 0.25 -> 5.5
# all same early 0.22 -> 7.1 converged...repeat->5.6...repeat->6.2...repeat->7.7
# leaky 0.5, early 0.20->6.1...repeat->7.4...
# leaky 0.3 early 0.25 batch 256 step 2.0, lrate 0.000001 (one extra zero)-> 5.04...repeat->5.4...repeat->5.6
# leaky 0.3 early 0.22 batch 256 step 2.0, lrate 0.000001 (one extra zero)-> 5.1...repeat->5.1
# leaky 0.3 early 0.18 batch 256 step 2.0, lrate 0.000001 (one extra zero)-> 5.1...repeat->5.2...repeat->5.2...
# leaky 0.3 early 0.18 batch 256 step 2.0, lrate 0.000001 (one extra zero) and inital_lrate 0.01->no converge...
# leaky 0.3 early 1.25 batch 256 step 2.0, lrate 0.000001 (one extra zero) and inital_lrate 0.01->no converge...
# leaky 0.1 early 0.18 batch 256 step 2.0, lrate 0.000001 (one extra zero) -> 5.1...repeat->nan...
# leaky 0.3 early 0.18 batch 256 step 4.0, lrate 0.00001 -> running...
# early 0.90, leaky 0.1 -> 5.1
# early 1.00, leaky 0.2 ->5.3
# early 0.8, leaky 0.2 ->4.5
# batch half initial lrate half 0.001, early 0.90, leaky 0.2 -> 4.7...4.6...
# batch half initial lrate half 0.001, early 0.90, leaky 0.9 -> 4.7...4.8
# batch half initial lrate half 0.001, early 0.90, leaky 0.01 -> 4.8
# batch half initial lrate half 0.001, early 1.1, leaky 0.1 -> 5.1...4.8...
# batch half initial lrate half 0.001, early 0.80, leaky 0.1 -> 4.9...4.6..
# batch half initial lrate half 0.001, early 0.75, leaky 0.1 -> 4.9...4.9...
# batch 256 initial lrate half 0.002, early 1.1, leaky 0.1 4 surrounding -> 18.9 (1.0)
# batch 256 initial lrate half 0.002, early 0.7, leaky 0.1 4 surrounding -> 6.8(0.47)
# batch 256 initial lrate half 0.002, early 0.5, leaky 0.1 4 surrounding -> 8.98(0.45)
# batch 256 initial lrate half 0.002, early 0.3, leaky 0.1 4 surrounding -> 5.48(0.27)
# batch 512 initial lrate half 0.0002, early 1.0, leaky 0.1 4 surrounding -> 5.6(0.94)...repeat..5.3(0.98)
# batch 512 initial lrate half 0.0002, early 0.9, leaky 0.1 4 surrounding -> 5.7...
# batch 512 initial lrate half 0.0002, 0.000001, early 1.0, leaky 0.1 4 surrounding -> 5.57...4.98....
# batch 512 initial lrate half 0.0002, 0.000001, early 0.9, leaky 0.1 4 surrounding -> 6.3...
# batch 512 initial lrate half 0.00002, 0.00001, step 4.0, early 0.9, leaky 0.1 4 surrounding (decoder motion fixed back to original)-> 5.2, repeat..7.2 (almost converged..)....
# batch 512 initial lrate half 0.0002, 0.000001, step 2.0, early 1.0, leaky 0.1 4 surrounding (added back layers) -> 4.92...repeat...5.7...
# batch 512 initial lrate half 0.0002, 0.000001, step 2.0, early 1.2, leaky 0.1 4 surrounding -> running....
# chnage leaky
# Sen server 3rd terminal
import os
...
# Path for local folder
# folderName = '/home/saptarshi/PythonCode/Junction/data/Surrounding4RelativeDecoderJuncEligibleCheck'     # old one without check ..... Surrounding4RelativeDecoderJuncDist
# Path for Sen Server folder
# folderName ='/media/disk1/sap/Junction/Surrounding4RelativeDecoderJuncDist'                 #Surrounding4RelativeDecoderJuncDist
# Path for Big Screen Server folder
folderName = '/home/sap/Sap/Junction/QuickDataNew'
# Path for small Server folder
# folderName = '/home/sap/Junction/QuickData'

# Specify the result file to store each sample error
resultFileName = 'V23ModidiedDecoder132.txt'
...
# Validation vehiles
totalVehileCount = 30 #1600 # 2200
validationVehicleCount = 10 # #400
