###############################################################################################################
# This one is training and testing on the Sighthill data. This one also has the simulated mis-detection and frame
# drop code inside. But this can be supressed using the "frameDropRate" variable. Set it to zero for perfect detection
# or zero simulated mis-detection. This version will be used to train on perfect data and then test on the "detected
# data using Marcel's pre-trained model, GM-PHD tracker and finally the pre-trained trajectory prediction model",
# the End to End pipeline (DTP). This is to compare the performance difference between prefect detection and End to
# End pipeline (DTP). 
###############################################################################################################
# Initially trying with zero mis-detection and perfect data for both training and testing with fixed set of
# validation vehicle IDs to finalize the RMSE error (*should be close to 4.5*). Then the same vehicle IDS will be
# used to get the RMSE with the DTP settings (means detected and tracked trajectories from Marcel's detection and
# GM-PHD filter saved in a csv file) to compare the peformance. This the final goal
# Step 1. -> Finalize the validation vehicles and network weight in perfect detection to get the old 4.5 (almost) error -> Trying Now...
# Step 2. -> Use the same network weight and same val vehicles but with detected and tracked trajs to get the DTP RMSE error -> ......
# *** Comapre the Particle Filter libary with the FilterNet code (new one) to check any genuine modifications....**** 
###############################################################################################################
...
mapImagePath = '/home/saptarshi/PythonCode/Junction/Maps/SightHillMap.png'

# Model paths
encoderFilePath = '/home/saptarshi/PythonCode/Junction/NavtechDTPModels/SighthillModels/DTPSighthillEncoderV2.h5'
decoderFilePath = '/home/saptarshi/PythonCode/Junction/NavtechDTPModels/SighthillModels/DTPSighthillDecoderV2.h5'

# Train or test the model
trainStr = 'Train'
testStr = 'Test'
trainOrTest = testStr   #   best training till now... 
...
leakyAlphaValue = 0.8   # dont change, for smooth traj... 0.8
dropOutFrac = 0.2
batchSize = 16   #16  #16    # 16  32 #64
nepochs = 60
surroudingCarCounts = 3   # originally 3
actualSurroudingCarCount = 3  # This is to show other cars as well
totalInputFeature = (actualSurroudingCarCount+1)*inputFeatureWithoutID
totalDecoderFeature = (actualSurroudingCarCount+1)*decoderInputFeature
maxSurroudingDist = 150     # orginally 150, increasing the value to see far cars..
repeatCount = 1
frameDropRate = 0 #  No need to simulate mis-detections for DTP
...
########### Weight Ratios #################
###########################################
# 0.342,0.223,0.435 -> 0.80/3.52/7.96.... 0.93/4.32/11.3...1.02/4.8/10.8...
# 0.345,0.22,0.435 -> 1.0,3.95,9.9....0.0032,3.5,7.9....0.40,3.8,71.9......
# 0.35,0.2,0.45 -> 0.002,0.99,6.8....0.76,3.43,6.3....(^-5),1.15,8.10...
# 0.345,0.2,0.455 -> ...1.00,4.94,7.86...0.16,2.72,7.12......(^-4,1.1,8.2)..
# 0.345,0.19,0.465 -> 0.96,4.05,11.92...0.06,3.1,7.8....1.19,4.2,8.61....
# 0.345,0.21,0.445 -> 0.17,1.48,12.7...0.92,4.48,27.8...1.00,3.26,5.53
# 0.345,0.2,0.455 -> 0.05,1.5,12.8...1.01,3.6,6.25... (^-5),1.00,8.15... error (9.2,9.1)
# 0.395,0.2,0.405 (step 15.0, leaky 0.8, extra BatchNorm than alvie) ->( 10^-4),1.1,8.2....error 9.0,8.6
# 0.39,0.2,0.41 -> (0.0011,1.4,7.9). (error 10.1,10.6) 
# 0.35,0.2,0.45 -> (0.0017,1.5,8.82) error (11,10)
# 0.345,0.2,0.455 -> 0.2,1.2,7.1   error (12,13)
# 0.346,0.2,0.454 -> class loss 0.5...0.5
# 0.348,0.2,0.452 -> (^-4),1.1,5.2...
# change step and leaky remove batchNorm....
##########################################


# Main road
laneLines =   [[140,418,456,427], [130,458,583,470], [456,427,1106,450], [583,470,1097,484], [458,427,466,89],                                       [580,470,598,1015],                    [550,540,100,523],                     [621,551,1069,563]]
...
# validationVehicleList = [58,157,57,20,36,11,25,52,6,25,38,50,13,101,105,68,112,47,94,115,137,155]     # Old list for train
# use 57 or 59 for right turn downward...



# Selected the best possible vehicles as validation to see the final error close to 4.4
# With this list it is almost 4.4, but few overlap between traing and testing, so retrainng with the new list to make the 
# the final error is 4.4 and ligit. So we can comapre with the DTP pipeline...
# One round achived  with the following vehicles and the models are save 
# in /home/saptarshi/PythonCode/Junction/NavtechDTPModels/SighthillModels/BestDTPSighthill location
# THe vehicle list: 58,63,81,57,20,36,11,25,52,25,38,111,32,122,123,112,50,126,89,92


# # # # validationVehicleList = [157,58,57,20,36,11,25,52,25,38,111,122,123,112,50,126,89,92,66,31,33]   # removed 47,66 and added 63 81...  # 12  11 validation new list for train  58,47,66,57, 86 error 4.41.. best..
# validationVehicleList = [4,98,124,57,59,94,115,152,158,66,63,127,155,111,129,132,134,161,170,171,173,135,138,139,140,147,149,152]
# Turn vehicles 4,115,127,66  extras 57,59,94,152,158,157
# validationVehicleList = [4,115,127,66]
validationVehicleList = [4,115,127,66,152,157,58,20,36,11,25,52,25,38,111,122,123,112,50,126,89,92,33] 
# error 5.4 5.3 3.9 5.9 4.5 3.7 3.5 last round errors, changed dropout factor from 0.15 to 0.20
# repeatCount 10 Error, dropout back to 0.15 -> 4.55, 
# Training now with the above validation list and dropout change...
# The error flactuation probably beacuse of the dropout. Try increase the repeat count to make it consisten,
# like predict multiple times for each prediction. The thing which we do by dropout and distribution prediction
# Use the repeatCount varaible
# Error after retrain -> 6.6, 7.2, 7.0, 7.1, 6.7, 6.6
# Training with more dropout for better fitting, error -> running..
...
# validationVehicleList = [115,66,81,52,6,86,126]     #,20,36,11,25,52,6,25,38,111,32,122,123,112,50,126,86]  # for video...
# validationVehicleList = [20,36,11,25,52,6,25,38,111,121,122,123,112,50,126,86] 
# validationVehicleList = [4,57,115,124,157,59,94,158,98]
# validationVehicleList = [58,20,36,11,25,52,6,25,38,50,13,68,112,5,6,8,9,10,11,32,33,37,39,60,61,62,68,86,92]
# validationVehicleList = [58,20,36,11,25,52,6,25,38,50,13,68,112,5,6,8,9,10,11,32,33,37,39,60,61,62,68,86,92,109,111,121,122,123,126]  # best

ignoreList = [30,49,64,78,90,103,174,175] # 119
...
def SmoothTrajcetoryGeneration(poseList):
    ...
    for mdx in range(0,trajLength):
        ...
        if(frameDropRate != 0 and ((mdx%frameDropRate == 0))):
        ...
        if (self.annotations != None):
            for anotationIndex, object in enumerate(self.annotations):
                ...
                for trajIndex in range(3,TargetVehiclePoseLength):
                    ...
                    if(nearestLaneIndex == 6 or nearestLaneIndex == 7):
...
def DataPrep():
    ...
    for eachCar in carInfoList:
        ...
        for eachSurroundingCount in range(0,actualSurroudingCarCount+1):
    ...
    for eachTrainCar in carInfoList:
        ...
        for idx in range(historyTemporal,currentVehicleLength-futureTemporal):
            ...
            for jdx in range(idx-historyTemporal,idx):
                ...
                # Seperate the surrouding info from the target vehicle and select the actual surrunding cars as oppose to all of them
                # the extra surroudning cars are for the visualization purpose
                surroudingInfo = eachTrainCar[jdx][inputFeature:inputFeature+actualSurroudingCarCount*inputFeature]
                ...
                expectedLen = (actualSurroudingCarCount+1)*inputFeatureWithoutID
            ...
            # select the actual surrouding cars and ignore the extras as those are for visualization
            firstDecoderSurrounding = eachTrainCar[jdx][inputFeature:inputFeature+actualSurroudingCarCount*inputFeature]
            ...
            expectedLen = (actualSurroudingCarCount+1)*decoderInputFeature
            ...
            for kdx in range(idx,idx+futureTemporal):
                ...
                # select the actual surrouding cars and ignore the extras as those are for visualization
                surroudingDecoderInfo = eachTrainCar[kdx][inputFeature:inputFeature+actualSurroudingCarCount*inputFeature]
...
def TestModel(encoder_model,decoder_model):
    ...
    for eachTrainCar,eachTrainCarDropped in zip(carInfoList,carInfoListDropped):
        ...
        for idx in range(historyTemporal,currentVehicleLength-futureTemporal):
            ...
            for jdx in range(idx-historyTemporal,idx):
                ...
                resultMapImage = cv2.putText(resultMapImage, 'Red : Surroundings Cars', (750,140), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
                ...
                resultRadarImage = cv2.putText(resultRadarImage, 'Red : Surroundings Cars', (750,140), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
                ...
                # Maintain a flog to check how many surroudning cars added Once actual surrouding cars appended stop appending any further
                # Let it draw all the cars but differnet color for the actual once and other for the rest
                surroudungAlreadyAdded = 0
                ...
                for indexRange in range(0,len(surroudingInfo),inputFeature):
                    ...
                    # Check if actual surroudnig cars added
                    if(surroudungAlreadyAdded < actualSurroudingCarCount):
                        currentSurroundingInfo = [normSurroudingPoseX,normSurroudingPoseY,normSurroudingVx,normSurroudingVy,normSurroudingLaneDist,normSurroudingLaneNumber]
                        tempInput.extend(currentSurroundingInfo)

                    # Increment the flag after eveyr surroudning vehicle added
                    surroudungAlreadyAdded = surroudungAlreadyAdded + 1
                    ...
                    if(jdx>0):
                        ...
                        if(surroudungAlreadyAdded <= actualSurroudingCarCount):
                            # selected surrounding color code
                            surroudingColor = (0,0,255)
                        else:
                            # extra surrounding color code
                ...
                if(jdx > 0):
                    ...
                    angleColor = (238,130,238) # target car color violet
                    surroudingtrajColor = (0,75,150)
                    # Check the movement info and decide the traj color
                    if(currentMovement == 0):
                        angleColor = (238,130,238)

                    # # resultRadarImage = cv2.line(resultRadarImage, (prevPoseX, prevPoseY), (currentPoseX, currentPoseY), surroudingtrajColor, lineThickness)
                    # # resultMapImage = cv2.line(resultMapImage, (prevPoseX, prevPoseY), (currentPoseX, currentPoseY), surroudingtrajColor, lineThickness)
            ...
            for kdx in range(idx,idx+futureTemporal):
                ...
                # Testing the relation between motion and pose
                currentMotionX = eachTrainCar[kdx-1][velXIndex]
                currentMotionY = eachTrainCar[kdx-1][velYIndex]
                groundTruthVel.append([currentMotionX,currentMotionY])
            ...
            for repeat in range(0,repeatCount):
                ...
                # Select only target and nearest actual surroudning cars
                target_seq = np.array(firstDecoderInput[:(actualSurroudingCarCount+1)*decoderInputFeature]).reshape(1,1,totalDecoderFeature)
                ...
                for t in range(futureTemporal):
                    ...
                    if(t<(futureTemporal-1)):
                        ...
                        # Select only nearest actual surrounding cars
                        currentSurroudingDecoder = surroudingDecoder[t][:actualSurroudingCarCount*decoderInputFeature]
                ...
                # Create a dictionary to maintain surrounding vehicles last pose against ID to draw the traj
                surroundingLastPoseDict = dict()
                ...
                for intitalIdIndex in range(0,len(initialSurroundingCars),3):
                    intialPoseX = initialSurroundingCars[intitalIdIndex]  # First item in the surroudingDecoderDrawList is poseX
                    intialPoseY = initialSurroundingCars[intitalIdIndex+1]  # Second item in the surroudingDecoderDrawList is poseY
                    initialId = initialSurroundingCars[intitalIdIndex+2] # Third item in the surroudingDecoderDrawList is ID
                    initialIdList.append(initialId)
                    surroundingLastPoseDict[str(initialId)] = [intialPoseX,intialPoseY]
                ...
                for drawIndex in range(1,predPoseLength):
                    ...
                    resultMapImage = cv2.putText(resultMapImage, maneuverStr, (750,230), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,0), 2, cv2.LINE_AA)
                    resultRadarImage = cv2.putText(resultRadarImage, maneuverStr, (750,230), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,0), 2, cv2.LINE_AA)

                    # Predicted trajectory color is Blue and Ground truth trajectory color is Green
                    predColor = (255,0,0)
                    trueColor = (0,255,0)
                    ...
                    # Maintain a flag to draw the nearest actual surrounding cars with different color
                    surroudungAlreadyAdded = 0
                    actualColor = (0,0,255)
                    extraColor = (0,255,255)
                    ...
                    for decodeDrawIndex in range(0,len(currentDrawSurrouding),3):  # Add the inputFeature as the index jump as the carInfo is prepered for input format, 3 jump beacuse there are 3 items in the draw list poseX, poseY and ID
                        ...
                        # # # # # # Not needed anymore as we are getting the last pose from the dict
                        ...
                        # Draw the surrouding traj on the radar and map image diff approach
                        if(currentSurroudingPoseX != 0 and currentSurroudingPoseY != 0):
                            if(currentID in initialIdList):
                                # Get the previous position from the dictionary
                                prevSurroudingPoseX = surroundingLastPoseDict[str(currentID)][0] # In surroundingLastPoseDict for each value first item is poseX
                                prevSurroudingPoseY = surroundingLastPoseDict[str(currentID)][1] # In surroundingLastPoseDict for each value second item is poseY
                                # Update the dictionary
                                surroundingLastPoseDict[str(currentID)] = [currentSurroudingPoseX,currentSurroudingPoseY]
                                if(prevSurroudingPoseX != 0 and prevSurroudingPoseY != 0):
                                    if(surroudungAlreadyAdded < actualSurroudingCarCount):
                                        resultRadarImage = cv2.line(resultRadarImage, (prevSurroudingPoseX, prevSurroudingPoseY), (currentSurroudingPoseX, currentSurroudingPoseY), actualColor, lineThickness)
                                        resultMapImage = cv2.line(resultMapImage, (prevSurroudingPoseX, prevSurroudingPoseY), (currentSurroudingPoseX, currentSurroudingPoseY), actualColor, lineThickness)
                                        surroudungAlreadyAdded = surroudungAlreadyAdded + 1
                                    else:
                                        resultRadarImage = cv2.line(resultRadarImage, (prevSurroudingPoseX, prevSurroudingPoseY), (currentSurroudingPoseX, currentSurroudingPoseY), extraColor, lineThickness)
                                        resultMapImage = cv2.line(resultMapImage, (prevSurroudingPoseX, prevSurroudingPoseY), (currentSurroudingPoseX, currentSurroudingPoseY), extraColor, lineThickness)

                            # else:
                            #     # Cannot draw trajectories whihch arent in the etection before prediction...
                            #     print('New id in tracker!!!')
                                # # # # if not (currentID in newIdListDrawn):
                                # # # #     # Add this entry in the dictionary
                                # # # #     surroundingLastPoseDict[str(currentID)] = [currentSurroudingPoseX,currentSurroudingPoseY]
                                # # # #     # Draw the new car
                                # # # #     resultRadarImage = cv2.circle(resultRadarImage, (currentSurroudingPoseX, currentSurroudingPoseY), 10, (255,255,0), lineThickness)
                                # # # #     resultMapImage = cv2.circle(resultMapImage, (currentSurroudingPoseX, currentSurroudingPoseY),10, (255,255,0), lineThickness)
                                # # # #     newIdListDrawn.append(currentID)

                                # # # # # Get the previous pose for this new ID from the dict
                                # # # # prevSurroudingPoseX = surroundingLastPoseDict[str(currentID)][0] # In surroundingLastPoseDict for each value first item is poseX
                                # # # # prevSurroudingPoseY = surroundingLastPoseDict[str(currentID)][1] # In surroundingLastPoseDict for each value second item is poseY

                                # # # # # Draw the surroundning tajectories based on actual and exta colors
                                # # # # if(surroudungAlreadyAdded < actualSurroudingCarCount):
                                # # # #     resultRadarImage = cv2.line(resultRadarImage, (prevSurroudingPoseX, prevSurroudingPoseY), (currentSurroudingPoseX, currentSurroudingPoseY), actualColor, lineThickness)
                                # # # #     resultMapImage = cv2.line(resultMapImage, (prevSurroudingPoseX, prevSurroudingPoseY), (currentSurroudingPoseX, currentSurroudingPoseY), actualColor, lineThickness)
                                # # # #     surroudungAlreadyAdded = surroudungAlreadyAdded + 1
                        ...
                        # # # # # Draw the surrouding traj on the radar and map image old approach
                        # # # # if(currentSurroudingPoseX != 0 and currentSurroudingPoseY != 0 and prevSurroudingPoseX != 0 and prevSurroudingPoseY != 0):
                        # # # #     if(currentID == prevID):
                        # # # #         if(currentID in initialIdList):
                        # # # #             resultRadarImage = cv2.line(resultRadarImage, (prevSurroudingPoseX, prevSurroudingPoseY), (currentSurroudingPoseX, currentSurroudingPoseY), (0,255,255), lineThickness)
                        # # # #             resultMapImage = cv2.line(resultMapImage, (prevSurroudingPoseX, prevSurroudingPoseY), (currentSurroudingPoseX, currentSurroudingPoseY), (0,255,255), lineThickness)
                        # # # #         else:
                        # # # #             if not (currentID in newIdListDrawn):
                        # # # #                 # draw the car
                    ...
                    # # #     imageSaveFilePath = '/home/saptarshi/PythonCode/Junction/CaseStudy/case3/' + str(localSaveCount) + '.png'
    ...
    print(finalMotionErrorArray/(errorCount))
...
def __main__():
    ...
    if(trainOrTest == trainStr):
        ...
        # Save the model
        encoder_model.save_weights(encoderFilePath)
        decoder_model.save_weights(decoderFilePath)


    elif(trainOrTest == testStr):
        print('Starting testing !!!')

        encoder_model.load_weights(encoderFilePath)
        decoder_model.load_weights(decoderFilePath)
