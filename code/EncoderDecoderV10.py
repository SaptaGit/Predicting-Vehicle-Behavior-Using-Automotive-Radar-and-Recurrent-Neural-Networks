...
# Path for Sen Server folder
# folderName = '/media/disk1/sap/Junction/Lankershim8SurroundingSen'
# Path for Big Screen Server folder
folderName = '/home/sap/Sap/Junction/Lankershim1'

# Specify the result file to store each sample error
resultFileName = 'EncoderDecoderV10Early.txt'
...
if __name__ == '__main__':
    ...
    outputFeatures = 6 #(3 class + 1 velocity + 2 pose )
    inputFeatures = 6    #(1 velocity + 2 pose + direction + movement + laneID)
    ...
    if (features != (inputFeatures*(surroudingCarCounts+1))):
        print('XTrain shape mismatch')
        print('Expected Shape :' + str((inputFeatures*(surroudingCarCounts+1))))
    ...
    decoder_dense10a = Dense(1024, activation='relu')
    decoder_output1 = decoder_dense10a(decoder_outputs)
    decoder_dense10 = Dense(512, activation='relu')
    ...
    decoder_dense20a = Dense(1024, activation='relu')
    decoder_output2 = decoder_dense20a(decoder_output2)
    decoder_dense20 = Dense(512, activation='relu')
    ...
    for mdx,key in enumerate(allFrameKeys):
        ...
        for eachEligibleKey in eligibleVehicleKeys:
            ...
            for udx, eachInputInfo in enumerate(inputInfo):
                ...
                # If all surrounding cars are removed perfrom a check to raise warinig
                if(len(surroudingCarIds) == 0):
                    print('Zero Surrounding cars!!!!!!')                             
