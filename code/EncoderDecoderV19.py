...
# Path for local folder
# folderName = '/home/saptarshi/PythonCode/Junction/Surrounding5RelativeDecoderJuncEligibleCheck'
# Path for Sen Server folder
folderName ='/media/disk1/sap/Junction/Surrounding4RelativeDecoderJuncDistEligibilityCheck'
# Path for Big Screen Server folder
# folderName = '/home/sap/Sap/Junction/Surrounding6RelativeDecoderJuncDistEligibilityCheck'
...
resultFileName = 'V19RelativeDecoderJuncDistEligibilityCheck3.txt'
...
def ValidationSetPredict(encoder_model,decoder_model,XVal,decoderValInput,YPoseVal, maxRealtiveX, maxRealtiveY):
    ...
    for pdx,eachXVal in enumerate(XVal):
        ...
        # Extend the same state for three different Decoder
        extendedState = [state[0], state[1], state[2], state[3], state[0], state[1], state[2], state[3], state[0], state[1], state[2], state[3]]

        # Perfrom the sequential prediction
        for t in range(futureTemporal):
            # predict next Features
            classPred, velcoityPred, posePred, h1_class, c1_class, h2_class, c2_class, h1_vel, c1_vel, h2_vel, c2_vel,h1_pose, c1_pose, h2_pose, c2_pose = decoder_model.predict([target_seq] + extendedState)
            ...
            # update state
            extendedState = [h1_class, c1_class, h2_class, c2_class, h1_vel, c1_vel, h2_vel, c2_vel, h1_pose, c1_pose, h2_pose, c2_pose]
...
if __name__ == '__main__':
    ...
    decoder_lstm_class_1 = LSTM(n_units, return_sequences=True, return_state=True)
    decoder_outputs_class, _, _ = decoder_lstm_class_1(decoder_inputs, initial_state=encoder_states1)
    # Second Decoder LSTM Layer
    decoder_lstm_class_2 = LSTM(n_units, return_sequences=True, return_state=True)
    decoder_outputs_class, _, _ = decoder_lstm_class_2(decoder_outputs_class, initial_state=encoder_states2)

    # Velocity decoder
    # First Decoder LSTM Layer
    decoder_lstm_vel_1 = LSTM(n_units, return_sequences=True, return_state=True)
    decoder_outputs_vel, _, _ = decoder_lstm_vel_1(decoder_inputs, initial_state=encoder_states1)
    # Second Decoder LSTM Layer
    decoder_lstm_vel_2 = LSTM(n_units, return_sequences=True, return_state=True)
    decoder_outputs, _, _ = decoder_lstm_vel_2(decoder_outputs_vel, initial_state=encoder_states2)

    # Position decoder
    # First Decoder LSTM Layer
    decoder_lstm_pose_1 = LSTM(n_units, return_sequences=True, return_state=True)
    decoder_outputs_pose, _, _ = decoder_lstm_pose_1(decoder_inputs, initial_state=encoder_states1)
    # Second Decoder LSTM Layer
    decoder_lstm_pose_2 = LSTM(n_units, return_sequences=True, return_state=True)
    decoder_outputs_pose, _, _ = decoder_lstm_pose_2(decoder_outputs_pose, initial_state=encoder_states2)
    ...
    decoder_output1 = decoder_dense10a(decoder_outputs_class)
    ...
    decoder_output2 = decoder2_concat([decoder_outputs_vel,classOut])
    ...
    decoder_output3 = decoder3_concat([decoder_outputs_pose,classOut,velocityConcat])
    ...
    decoder_state_input_h_class_1 = Input(shape=(n_units,))
    decoder_state_input_c_class_1 = Input(shape=(n_units,))
    decoder_state_input_h_class_2 = Input(shape=(n_units,))
    decoder_state_input_c_class_2 = Input(shape=(n_units,))
    decoder_states_inputs_class_1 = [decoder_state_input_h_class_1, decoder_state_input_c_class_1]
    decoder_states_inputs_class_2 = [decoder_state_input_h_class_2, decoder_state_input_c_class_2]
    decoder_states_inputs_class = [decoder_state_input_h_class_1, decoder_state_input_c_class_1,decoder_state_input_h_class_2, decoder_state_input_c_class_2]

    # Inference decoder for Class
    decoder_outputs_class, state_h1_class, state_c1_class = decoder_lstm_class_1(decoder_inputs, initial_state=decoder_states_inputs_class_1)
    decoder_outputs_class, state_h2_class, state_c2_class = decoder_lstm_class_2(decoder_outputs_class, initial_state=decoder_states_inputs_class_2)

    # define inference decoder
    decoder_state_input_h_vel_1 = Input(shape=(n_units,))
    decoder_state_input_c_vel_1 = Input(shape=(n_units,))
    decoder_state_input_h_vel_2 = Input(shape=(n_units,))
    decoder_state_input_c_vel_2 = Input(shape=(n_units,))
    decoder_states_inputs_vel_1 = [decoder_state_input_h_vel_1, decoder_state_input_c_vel_1]
    decoder_states_inputs_vel_2 = [decoder_state_input_h_vel_2, decoder_state_input_c_vel_2]
    decoder_states_inputs_vel = [decoder_state_input_h_vel_1, decoder_state_input_c_vel_1,decoder_state_input_h_vel_2, decoder_state_input_c_vel_2]

    # Inference decoder for Velocity
    decoder_outputs_vel, state_h1_vel, state_c1_vel = decoder_lstm_vel_1(decoder_inputs, initial_state=decoder_states_inputs_vel_1)
    decoder_outputs_vel, state_h2_vel, state_c2_vel = decoder_lstm_vel_2(decoder_outputs_vel, initial_state=decoder_states_inputs_vel_2)

    # define inference decoder
    decoder_state_input_h_pose_1 = Input(shape=(n_units,))
    decoder_state_input_c_pose_1 = Input(shape=(n_units,))
    decoder_state_input_h_pose_2 = Input(shape=(n_units,))
    decoder_state_input_c_pose_2 = Input(shape=(n_units,))
    decoder_states_inputs_pose_1 = [decoder_state_input_h_pose_1, decoder_state_input_c_pose_1]
    decoder_states_inputs_pose_2 = [decoder_state_input_h_pose_2, decoder_state_input_c_pose_2]
    decoder_states_inputs_pose = [decoder_state_input_h_pose_1, decoder_state_input_c_pose_1,decoder_state_input_h_pose_2, decoder_state_input_c_pose_2]

    # Inference decoder for Pose
    decoder_outputs_pose, state_h1_pose, state_c1_pose = decoder_lstm_pose_1(decoder_inputs, initial_state=decoder_states_inputs_pose_1)
    decoder_outputs_pose, state_h2_pose, state_c2_pose = decoder_lstm_pose_2(decoder_outputs_pose, initial_state=decoder_states_inputs_pose_2)


    decoder_states = [state_h1_class, state_c1_class, state_h2_class, state_c2_class,state_h1_vel, state_c1_vel, state_h2_vel, state_c2_vel,state_h1_pose, state_c1_pose, state_h2_pose, state_c2_pose]
    decoder_states_inputs = [decoder_state_input_h_class_1, decoder_state_input_c_class_1, decoder_state_input_h_class_2, decoder_state_input_c_class_2, decoder_state_input_h_vel_1, decoder_state_input_c_vel_1, decoder_state_input_h_vel_2, decoder_state_input_c_vel_2, decoder_state_input_h_pose_1, decoder_state_input_c_pose_1, decoder_state_input_h_pose_2, decoder_state_input_c_pose_2]
    ...
    model.fit([XTrain,decoderTrainInput], [YClassTrain,YVelTrain,YPoseTrain], batch_size=batchSize, epochs=nepochs, verbose=1, validation_data=([XVal,decoderValInput],[YClassVal,YVelVal,YPoseVal]), callbacks=callbacks_list)
