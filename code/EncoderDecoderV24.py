# leaky 0.5 early 0.17 but old uncheck data batch 256 still good epochs_drop = 10.0 (from 6.0, from 2.0)
# Last time converged and result -> 9.0
# old data, New arch, early 0.15, batch 128, only 2 BatchNorms for each section and common batchnorm, epochs_drop = 6.0 with extra zero for lower cap, RMSProp -- running
# Big Screen server 2nd terminal
# no converge error 5.71
# Adam convergd error 6.0 early 0.15
# rmsprop intial step 0.01 insted of 0.001 class loss huge (15,1.26, 1.26 ), retry...
# step 5.0 0.001 early 0.25 leaky 0.3 -> 5.3 converged 35/150
# all same early 0.22 -> 5.6...repeat->5.4...repeat->5.3...repeat->5.8...repeat->5.4
# leaky 0.5, early 0.20->6.05...repeat->5.8...repeat->6.08...
# leaky 0.5 early 0.25 batch 256 step 5.0, lrate 0.000001 (one extra zero)->5.6...repeat->5.4
# leaky 0.5 early 0.22 batch 256 step 5.0, lrate 0.000001 (one extra zero)-> 5.6...repeat->5.35...repeat->5.6
# leaky 0.5 early 0.18 batch 256 step 5.0, lrate 0.000001 (one extra zero)-> 5.8....repeat->5.3...repeat->5.8
# leaky 0.5 early 1.25 batch 256 step 5.0, lrate 0.000001 (one extra zero)-> 22.... major bad
# leaky 0.5 early 0.18 batch 256 step 2.0, lrate 0.000001 (one extra zero) and inital_lrate 0.01->no converge...
# leaky 0.5 early 1.25 batch 256 step 2.0, lrate 0.000001 (one extra zero) and inital_lrate 0.01->no converge...
# leaky 0.1 early 0.18 batch 256 step 2.0, lrate 0.000001 (one extra zero) -> 4.97...
# leaky 0.2 early 0.18 batch 256 step 3.0, lrate 0.00001, motion, shuffle->
# early 0.90, leaky 0.1 -> 4.1
# early 1.00, leaky 0.2 -> 4.9
# early 0.8, leaky 0.2 -> 5.5
# batch half initial lrate half 0.001, early 0.90, leaky 0.2 -> 4.4...4.6...
# batch half initial lrate half 0.001, early 0.90, leaky 0.9 -> 5.8...5.1...
# batch half initial lrate half 0.001, early 0.90, leaky 0.01 -> 4.9
# batch half initial lrate half 0.001, early 1.1, leaky 0.1 -> 5.2...5.7...
# batch half initial lrate half 0.001, early 0.80, leaky 0.1 -> 5.2...4.9...
# batch half initial lrate half 0.001, early 0.75, leaky 0.1 -> 4.7...5.1...
# batch 256 initial lrate half 0.002, early 1.1, leaky 0.1 5 surrounding-> 7.0 (0.85)
# batch 256 initial lrate half 0.002, early 0.7, leaky 0.1 5 surrounding-> 10.6(0.49)
# batch 256 initial lrate half 0.002, early 0.5, leaky 0.1 5 surrounding-> 5.85(0.34)
# batch 256 initial lrate half 0.002, early 0.3, leaky 0.1 5 surrounding-> 5.5(0.20)...
# batch 512 initial lrate half 0.0002, early 1.0, leaky 0.1 5 surrounding-> 5.95(0.97)...repeat..6.75(0.97)
# batch 512 initial lrate half 0.00002, 0.000001 early 1.0, leaky 0.1 5 surrounding-> 4.62...5.85...
# batch 512 initial lrate half 0.0002, 0.000001 early 1.0, leaky 0.1 5 surrounding-> 5.81...repeat...5.5...
# batch 512 initial lrate half 0.0002, 0.000001 early 1.0, leaky 0.1 5 surrounding (decoder motion fixed back to original) -> 5.6... 5.1..running...
# batch 512 initial lrate half 0.00002, 0.000001 early 1.0, leaky 0.1 5 surrounding -> 5.9. (not converged)..not needed........
# batch 512 initial lrate half 0.0002, 0.000001 early 1.0, step 2.0, leaky 0.1 5 surrounding (added back layers) -> 4.71...repeat...4.7 
# batch 512 initial lrate half 0.0002, 0.000001 early 1.2, step 2.0, leaky 0.1 5 surrounding (added back layers) -> running...
# keep 0.00002 0.000001 change step 2.0 to 4.0... if not coverged.....

# change leaky
# if converge early 0.15
...
# folderName ='/media/disk1/sap/Junction/Surrounding4RelativeDecoderJuncDist'
# Path for Big Screen Server folder
folderName = '/home/sap/Sap/Junction/EligibilityCheckRemoved5Surrouding'
# Path for small Server folder
# folderName = '/home/sap/Junction/Surrounding4RelativeDecoderJuncDistEligibilityCheck'

# Specify the result file to store each sample error
resultFileName = 'V24ModidiedDecoder92.txt'
...
def ValidationSetPredict(encoder_model,decoder_model,XVal,decoderValInput,YPoseVal, maxRealtiveX, maxRealtiveY):
    ...
    for pdx,eachXVal in enumerate(XVal):
        ...
        for t in range(futureTemporal):
            ...
            # /10 as the velocity is feet/sec but needed geet/frame and 10 fps
