...
# Path for local folder
# folderName = '/home/saptarshi/PythonCode/Junction/Lankershim4RelativeJunc'
# Path for Sen Server folder
# folderName ='/media/disk1/sap/Junction/Surrounding5RelativeExtraCheck10Sec'
# Path for Big Screen Server folder
folderName = '/home/sap/Sap/Junction/Surrounding5RelativeExtraCheckEligibility10Sec'
...
resultFileName = 'V16RelativeJuncDist17.txt'
    ...
    if (otherVechilesCount <= (surroudingCarCounts+1)):
        ...
        for eachOtherVechiles in otherVechiles:
            ...
            if(inputOrDecoder == inputStr):
                ...
                if(otherDist>maximumSurroundingCarDist):
                    tempInput.extend([0,0,0,0,0,0])
                else:
                    lastInputClassInfo = MovementToClassForm(otherMovement)
                    tempInput.extend([otherRelativeX,otherRelativeY,otherVelocity,lastInputClassInfo[0],lastInputClassInfo[1],lastInputClassInfo[2]])
            else:
...
def ProcessByVehicle(processItme):
    ...
    for idx in range(historyTemporal,currentVehicleLength-futureTemporal):
        ...
        for kdx in range(idx,idx+futureTemporal):
            ...
            decoderTemp = [nextRelativeX,nextRelativeY,nextVelocity,nextMovementClassData[0],nextMovementClassData[1],nextMovementClassData[2]]
    ...
    for pdx,eachXVal in enumerate(XVal):
        ...
        for t in range(futureTemporal):
            ...
            if(t<(futureTemporal-1)):
                ...
                surroundingDecoder = decoderLocal[t+1][globalOutputFeatures:] # 6 -> target vehicle features
        ...
        elif(futureTemporal == 100):
            printList = np.around(np.array([finalError[0],finalError[4],finalError[9],finalError[14],finalError[19],finalError[24],finalError[29],finalError[34],finalError[39],finalError[44],finalError[49],finalError[54],finalError[59],finalError[64],finalError[69],finalError[74],finalError[79],finalError[84],finalError[89],finalError[94],finalError[99]])/count, 2)
        else:
            print('Unknown future temporal!!!')
...
class EarlyStoppingByLossVal(callbacks.Callback):
    def __init__(self, monitor='val_loss', value=0.15, verbose=0):
...
class PredictionInfos():
    def __init__(self, input = [], decoderInput = [], state = [], output=[], groundTruth = [], initialPose = []):
        self.input = input
...
if __name__ == '__main__':
    ...
    if(processOrRead == processStr):
        ...
        for ydx in range(len(decoderTrainInput)):
            for zdx in range(0,globalDecoderFeatures,globalOutputFeatures):
    ...
    for key in allFrameKeys:
        ...
        # eligibleVehicleKeys replace by  modifiedEligibleKeys
        for eachEligibleKey in modifiedEligibleKeys:
            ...
            for udx, eachInputInfo in enumerate(inputInfo):
                ...
                # eligibleVehicleKeys replaced by modifiedEligibleKeys
            ...
            for eachPredDictKey in predictionDict.keys():
                ...
                if(decoderSurroudingCarCount <= surroudingCarCounts):
                    for eachDecoderSurroundingCarID in decoderSurroudingCarIds:
                        ...
                        # Finally add the normalized values into the temp decoder input
                        if(otherDist>maximumSurroundingCarDist):
                            tempDecoderInput.extend([0,0,0,0,0,0])
                        else:
        ...
        for eachErrorPredDictKey in predictionDict.keys():
            ...
            if(futureTemporal == 50):
                ...
                print('Not known future temporal!!!!')
