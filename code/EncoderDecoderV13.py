...
# Path for local folder
# folderName = '/media/saptarshi/Elements/sap/Surrounding4RelativeJuncDist1'
# Path for Sen Server folder
# folderName ='/media/disk1/sap/Junction/Lankershim4SurroundingRelative'
# Path for Big Screen Server folder
# folderName = '/home/sap/Sap/Junction/Lankershim4SurroundingRelative'
# Path for small Server folder
folderName = '/home/sap/Junction/Surrounding5RelativeExtraCheckEligibility10Sec'

# Specify the result file to store each sample error
resultFileName = 'V13RelativeJuncDistEligiblity10Sec3.txt'
...
inputFeatureCount = 7  # 7 -> (poseX,poseY,velocity, LaneID, Movement, Direction, distance from junc)
...
def ValidationSetPredict(encoder_model,decoder_model,XVal,decoderValInput,YPoseVal, maxRealtiveX, maxRealtiveY):
    ...
    for pdx,eachXVal in enumerate(XVal):
        ...
        for t in range(futureTemporal):
            ...
            surroundingDecoder = decoderLocal[t][globalOutputFeatures:] # 6 -> target vehicle features
...
class EarlyStoppingByLossVal(callbacks.Callback):
    def __init__(self, monitor='val_loss', value=0.22, verbose=0):
