...
# Path for local folder
# folderName = '/home/saptarshi/PythonCode/Junction/Lankershim2'
# Path for Sen Server folder
# folderName = '/media/disk1/sap/Junction/Lankershim1'
# Path for Big Screen Server folder     Lankershim1  Lankershim8SurroundingBig Lankershim8Surrounding
folderName = '/home/sap/Sap/Junction/Lankershim1'

# Specify the result file to store each sample error
resultFileName = 'V12Early6DecayPer30epRateHalf8Car26.txt'
...
def TrainData(inputFileName):
    ...
    global dictByFrames, dictByVehicles, validationVehicles, mapper
...
def ValidationSetPredict(encoder_model,decoder_model,XVal,decoderValInput,YPoseVal):
    ...
    for pdx,eachXVal in enumerate(XVal):
        ...
        for t in range(futureTemporal):
            ...
            surroundingDecoder = decoderLocal[t][6:] # 6 -> target vehicle features
...
class EarlyStoppingByLossVal(callbacks.Callback):
    def __init__(self, monitor='val_loss', value=3, verbose=0):
...
if __name__ == '__main__':
    ...
    inputFeatures = globalInputFeatures    #(1 velocity + 2 pose + direction + movement + laneID)
    ...
    encoder1 = LSTM(n_units1, return_state=True, return_sequences=True)
    encoder_output, state_h1, state_c1 = encoder1(encoder_inputs)
    encoder_states1 = [state_h1, state_c1]
    # Second Encoder LSTM Layer
    encoder2 = LSTM(n_units2, return_state=True)
    ...
    decoder_lstm1 = LSTM(n_units1, return_sequences=True, return_state=True)
    decoder_outputs, _, _ = decoder_lstm1(decoder_inputs, initial_state=encoder_states1)
    # Second Decoder LSTM Layer
    decoder_lstm2 = LSTM(n_units2, return_sequences=True, return_state=True)
    ...
    decoder_state_input_h1 = Input(shape=(n_units1,))
    decoder_state_input_c1 = Input(shape=(n_units1,))
    decoder_state_input_h2 = Input(shape=(n_units2,))
    decoder_state_input_c2 = Input(shape=(n_units2,))
    ...
    ValidationSetPredict(encoder_model, decoder_model, XVal, decoderValInput, YPoseVal)
    ...
    frameCount = str(len(allFrameKeys))

    for mdx,key in enumerate(allFrameKeys):
        ...
        for eachEligibleKey in eligibleVehicleKeys:
            ...
            intialTime = inputInfo[0][6] # 0 for the first item and 6 is time index in TrackerDict list
            ...
            for udx, eachInputInfo in enumerate(inputInfo):
                ...
                # Remove vehicles with different time stamps 
                for hdx,eachTimeEligibleKey in enumerate(surroudingCarIds):
                    otherTime = trackerDict[eachTimeEligibleKey][0][6] # 0 for the first item and 6 is time index in TrackerDict list
        ...
        for eachErrorPredDictKey in predictionDict.keys():
            ...
            printList = np.around(np.array([finalError[0],finalError[4],finalError[9],finalError[14],finalError[19],finalError[24],finalError[29],finalError[34],finalError[39],finalError[44],finalError[49]])/count, 2)
            # print(printList)
            print('Current Frame : ' + str(mdx) + '/' + frameCount + ' ', end=' ')
            print(*printList, end='\r', flush=True)
