# leaky 0.5, early 0.16 (6.65) (->0.18 (error 6.21)) uncheck data batch 256 step decay with epochs_drop = 6.0 and added batchNorm 
# Adam(),  BatchNorm,  K.set_learning_phase for batchNorm, QuickData(small new data), batch -> 256, epochs_drop = 6.0, (File 22 Will have same but batch -> 128) early 0.17
# old data, No batchNorm, early 0.15, batch 1024, epochs_drop = 10.0 RMSProp() --> not fitted proporly, error 6.56
# old data, No batchNorm, early 0.15, batch 1024, epochs_drop = 10.0 Adam() --> fitted proporly error 7.8
# old data, No batchNorm, early 0.20, batch 1024, epochs_drop = 6.0 RMSProp(), ELU --> not fitted properly error 6.5
# old data, No batchNorm, early 0.20, batch 1024, epochs_drop = 6.0 Nadam(), ELU lrate 0.002 for Nadam--> fitted properly error 4.5 around   
# old data, No batchNorm, early 0.19, batch 1024, epochs_drop = 6.0 Nadam(), ELU lrate 0.001 for Nadam--> 4.8
# old data, No batchNorm, early 0.22, batch 2048, epochs_drop = 15.0 Nadam(), ELU lrate 0.002 for Nadam--> 5.3
# old data, No batchNorm, early 0.22, batch 2048, epochs_drop = 8.0 Nadam(), ELU lrate 0.002 for Nadam--> 4.5
# old data, No batchNorm, early 0.21, batch 2048, epochs_drop = 7.0 Nadam(), ELU lrate 0.002 for Nadam--> 4.91
# old data, No batchNorm, early 0.23, batch 2048, epochs_drop = 8.0 Nadam(), ELU lrate 0.002 for Nadam--> 4.81
# old data, No batchNorm, early 0.23, batch 2048, epochs_drop = 5.0 Nadam(), ELU lrate 0.002 for Nadam--> 4.69
# old data, No batchNorm, early 0.21, batch 2048, epochs_drop = 5.0 Nadam(), ELU lrate 0.002 for Nadam--> 4.41
# old data, No batchNorm, early 0.19, batch 2048, epochs_drop = 5.0 Nadam(), ELU lrate 0.002 for Nadam--> 4.49
# old data, No batchNorm, early 0.20, batch 2048, epochs_drop = 3.0 Nadam(), ELU lrate 0.002 for Nadam--> 4.70
# old data, No batchNorm, early 0.20, batch 2048, epochs_drop = 4.0 Nadam(), ELU lrate 0.002 for Nadam--> 4.6 (extra zero for <lrate)
# old data, No batchNorm, early 0.21, batch 2048, epochs_drop = 5.0 Nadam(), ELU lrate 0.002 for Nadam--> 5.5...repeat->4.7...repeat->...nan...repeat->4.7...repeat->4.58
# All same ELU alpha = 0.3-> 4.6...repeat->4.7
# All same ELU alpha = 0.2-> 4.6
# All same ELU alpha = 0.2 lrate 0.000001 (one extra zero) -> 4.7 around...
# leaky 0.1, batch 2048 early 0.22 step 5.0 initial_lrate 0.002, lrate 0.000001 (one extra zero), Nadam, motion, shuffle -> 
# leaky 0.1 -> 4.2
# leaky 0.8 -> 3.93...
# leaky 0.8, early 1.1 -> 3.99...4.17... 
# leaky 0.1, 5 surrouding, early 1.1 -> running...  to block gpu 
# Sen server 2nd terminal  
import os
os.environ["CUDA_VISIBLE_DEVICES"]="6"    # 6
import PIL
...
# Path for local folder
# folderName = '/home/saptarshi/PythonCode/Junction/data/Surrounding5RelativeDecoderJuncEligibleCheck'
...
# Path for Sen server folder
encoderModelFilename = '/media/disk1/sap/Junction/OldDataEncoder18V2.h5'
decoderModelFilename = '/media/disk1/sap/Junction/OldDataDecoder18V2.h5'
# Path for Big Screen Server folder
...
resultFileName = 'V18RelativeDecoderJuncDistEligibilityCheck127.txt'
...
def step_decay(epoch):
    initial_lrate = 0.002  # 0.001 for Adam and RMSProp , 0.002 for Nadam 
    drop = 0.5
    epochs_drop = 2.0
...
# #     initial_lrate = 0.002
# #     k = 0.03
# #     lrate = initial_lrate * exp(-k*(epoch-1))
# #     if lrate < 0.00001:
# #         lrate = 0.00001
# #     return lrate
