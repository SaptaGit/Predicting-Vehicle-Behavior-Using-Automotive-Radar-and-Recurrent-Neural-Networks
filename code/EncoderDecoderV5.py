...
def CreateVehicleAndFrameDict(loadFileName):
    ...
    # Calculate the local dx and dy for each time stamp and append to the end of the array as column
    allLocalX = datasetArray[:,localXIndex]
    allLocalY = datasetArray[:,localYIndex]
    localXDiffList = []
    localYDiffList = []
    runningIDForX = -1000
    runningIDForY = -1000

    for qdx, eachLocalX in enumerate(allLocalX):
        currentIDX = datasetArray[qdx,vehicleIDIndex]
        if(currentIDX != runningIDForX):
            localXDiffList.append(0) # for first item use velocity 0 at the moment
            runningIDForX = currentIDX
        else:
            currentXDiff = (allLocalX[qdx] - allLocalX[qdx-1])
            localXDiffList.append(currentXDiff)

    for pdx, eachLocalY in enumerate(allLocalY):
        currentIDY = datasetArray[pdx,vehicleIDIndex]
        if(currentIDY != runningIDForY):
            localYDiffList.append(0) # for first item use velocity 0 at the moment
            runningIDForY = currentIDY
        else:
            currentYDiff = (allLocalY[pdx] - allLocalY[pdx-1])
            localYDiffList.append(currentYDiff)

    localXDiffArray = np.array(localXDiffList)
    localYDiffArray = np.array(localYDiffList)
    
    datasetArray = np.hstack((datasetArray,localXDiffArray.reshape((len(localXDiffArray),1)),localYDiffArray.reshape((len(localYDiffArray),1))))


    
    # Normalize each feature columns
    normalizeIndexList = [localXIndex,velocityIndex,laneIDIndex,directionIndex,movementIndex,localYIndex,localdXIndex,localdYIndex]

    # Save the original min max value for further denormalization
    global minLocalX,maxLocalX,minLocalY,maxLocalY,minVel,maxVel,minLocaldX,maxLocaldX,minLocaldY,maxLocaldY
    ...
    minLocaldX = min(datasetArray[:,localdXIndex])
    maxLocaldX = max(datasetArray[:,localdXIndex])

    minLocaldY = min(datasetArray[:,localdYIndex])
    maxLocaldY = max(datasetArray[:,localdYIndex])
    ...
    for currentVehicle in finalVehicleKeys:
        ...
        for idx in range(historyTemporal,currentVehicleLength-futureTemporal):
            ...
            for jdx in range(idx-historyTemporal,idx):
                ...
                localdX = currentVehicleList[jdx][localdXIndex]
                localdY = currentVehicleList[jdx][localdYIndex]
                ...
                localXData.append([movement,velocity,localdX,localdY,laneID,direction])
            ...
            lastInputMovement = localXData[-1][0]
            lastInputClassInfo = MovementToClassForm(lastInputMovement)
            lastInputVelocity = localXData[-1][1]
            lastInputPoseX = localXData[-1][2]
            lastInputPoseY = localXData[-1][3]
            firstDecoderInput = [lastInputClassInfo[0],lastInputClassInfo[1],lastInputClassInfo[2],lastInputVelocity,lastInputPoseX,lastInputPoseY]
            ...
            for kdx in range(idx,idx+futureTemporal):
                ...
                nextLocaldX = currentVehicleList[kdx][localdXIndex]
                denormalizedNextLocalX = (nextLocaldX*(maxLocaldX-minLocaldX)+minLocaldX)
                nextLocaldY = currentVehicleList[kdx][localdYIndex]
                denormalizedNextLocalY = (nextLocaldY*(maxLocaldY-minLocaldY)+minLocaldY)
                ...
                decoderInputData.append([nextMovementClassData[0],nextMovementClassData[1],nextMovementClassData[2],nextVelocity,nextLocaldX,nextLocaldY])
...
if __name__ == '__main__':
    ...
    for pdx,eachXVal in  enumerate(XVal):
        ...
        lastMovementForPred = eachXVal[-1][0]
        lastClassInfoForPred = MovementToClassForm(lastMovementForPred)
        lastVelocityForPred = eachXVal[-1][1]
        lastPoseXForPred = eachXVal[-1][2]
        lastPoseYForPred = eachXVal[-1][3]
        firstDecoderInputForPred = [lastClassInfoForPred[0],lastClassInfoForPred[1],lastClassInfoForPred[2],lastVelocityForPred,lastPoseXForPred,lastPoseYForPred]

        target_seq = np.array(firstDecoderInputForPred).reshape(1,1,outputFeatures)
        ...
        for t in range(futureTemporal):
            ...
            # Normalize the predicted local poses for next instance prediction
            normalizedPredPoseX = (posePred[0][0][0]-minLocaldX)/(maxLocaldX-minLocaldX)
            normalizedPredPoseY = (posePred[0][0][1]-minLocaldY)/(maxLocaldY-minLocaldY)
