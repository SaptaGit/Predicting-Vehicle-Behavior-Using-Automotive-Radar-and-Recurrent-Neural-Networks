...
# folderName ='/media/disk1/sap/Junction/Surrounding5RelativeDecoderJuncDistEligibilityCheck'
# Path for Big Screen Server folder
# folderName = '/home/sap/Sap/Junction/Surrounding5RelativeDecoderJuncDistEligibilityCheck'
...
resultFileName = 'ModifiedDecoderV1.txt'
...
def ValidationSetPredict(encoder_model_copy, encoder_model,decoder_model_copy, decoder_model,XVal,decoderValInput,YPoseVal, maxRealtiveX, maxRealtiveY):
...
#def IntermediatePredictionForTraining(encoder_model,decoder_model,dictByFrames,dictByVehicles,mapperDict,vehicleFileName):
def IntermediatePredictionForTraining(encoder_model,decoder_model,vehicleFileName):

    finalXDataList = []
    finalDecoderInputList = []
    finalPoseOutputList = []
    finalVelocityOutputList = []
    finalMovementOutputList = []

    ###########################################
    # Add the side origin check.........................
    ###################################################



    # Read the training vehicle ID list:


    # Read the validation file (Replace with training.. for the time being)
    vehicleFileObj = open(vehicleFileName, "r")
    vehicleLoadedData = vehicleFileObj.readlines()
    vehicleList = []

    for eachVehicle in vehicleLoadedData:
        vehicleList.append(eachVehicle.rstrip())
    
    valFileObj.close()

    for eachRelevenatVehicle in vehicleList:
        ...
        for idx in range(historyTemporal,currentReleventVehicleLength-futureTemporal):
            ...
            # Initialize the local list for the current sample
            localXData = []
            decoderInputData = []
            localYMovementData = []
            localYVelData = []
            ...
            totalGroundTruthOutputList = predictionDict[targetUpdatedID].output
            localYMovementData = []
            localYVelData = []
            localYPoseData = []
            for eachGroundTurthOutput in totalGroundTruthOutputList:
                localYPoseData.append([eachGroundTurthOutput[0],eachGroundTurthOutput[1]])   # Output PoseX and PoseY index 0,1
                localYVelData.append(eachGroundTurthOutput[2])   # Output velocity index 2
                localYMovementData.append([eachGroundTurthOutput[3],eachGroundTurthOutput[4],eachGroundTurthOutput[5]])   # Output class0, class1, clas2 index 3,4,5
            ...
            # Once all check passed add the local lists to the final list
            finalXDataList.append(localXData)
            finalDecoderInputList.append(decoderInputData)
            finalPoseOutputList.append(localYPoseData)
            finalVelocityOutputList.append(localYVelData)
            finalMovementOutputList.append(localYMovementData)

    # Once all fininshed conver to array and return
    finalXDataArray = np.array(finalXDataList)
    finalDecoderInputArray = np.array(finalDecoderInputList)
    finalPoseOutputArray = np.array(finalPoseOutputList)
    finalVelocityOutputArray = np.array(finalVelocityOutputList)
    finalMovementOutputArray = np.array(finalMovementOutputList)

    return finalXDataArray,finalDecoderInputArray,finalPoseOutputArray,finalVelocityOutputArray,finalMovementOutputArray
...
if __name__ == '__main__':
    ...
    encoder_model.save('/home/saptarshi/PythonCode/Junction/encoder.h5')
    encoder_model_copy = load_model('/home/saptarshi/PythonCode/Junction/encoder.h5')
    decoder_model.save('/home/saptarshi/PythonCode/Junction/decoder.h5')
    decoder_model_copy = load_model('/home/saptarshi/PythonCode/Junction/decoder.h5', custom_objects={'minVelConst': minVelConst,'minMaxVelDiffConst' : minMaxVelDiffConst})
    # encoder_model.save('/home/saptarshi/PythonCode/Junction/encoder.h5')


    # # define inference COPY encoder
    # encoder_model_copy = Model(encoder_inputs, encoder_states)

    # for l_tg,l_sr in zip(encoder_model_copy.layers,encoder_model.layers):
    #     wk0=l_sr.get_weights()
    #     l_tg.set_weights(wk0)
    # print("model source was copied into model target")

    # ValidationSetPredict(encoder_model_copy, encoder_model, decoder_model_copy, decoder_model, XVal, decoderValInput, YPoseVal, maxRealtiveX, maxRealtiveY)


    # trainXDataArray,trainDecoderInputArray,trainPoseOutputArray,trainVelocityOutputArray,trainMovementOutputArray = IntermediatePredictionForTraining(encoder_model,decoder_model,dictByFrames,dictByVehicles,mapperDict,trainingFileName)
    # valXDataArray,valDecoderInputArray,valPoseOutputArray,valVelocityOutputArray,valMovementOutputArray = IntermediatePredictionForTraining(encoder_model,decoder_model,dictByFrames,dictByVehicles,mapperDict,trainingFileName)


    trainXDataArray,trainDecoderInputArray,trainPoseOutputArray,trainVelocityOutputArray,trainMovementOutputArray = IntermediatePredictionForTraining(encoder_model,decoder_model,trainingFileName)
    valXDataArray,valDecoderInputArray,valPoseOutputArray,valVelocityOutputArray,valMovementOutputArray = IntermediatePredictionForTraining(encoder_model,decoder_model,trainingFileName)

    # re train for the updated decoder                                                                                                                                                                      valXDataArray,valDecoderInputArray,valPoseOutputArray,valVelocityOutputArray,valMovementOutputArray                                      
    model.fit([trainXDataArray,trainDecoderInputArray], [trainMovementOutputArray,trainVelocityOutputArray,trainPoseOutputArray], batch_size=batchSize, epochs=4, verbose=1, validation_data=([valXDataArray,valDecoderInputArray],[valMovementOutputArray,valVelocityOutputArray,valPoseOutputArray]), callbacks=callbacks_list)
