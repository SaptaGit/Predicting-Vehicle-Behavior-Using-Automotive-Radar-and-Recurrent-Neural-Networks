###############################################################################################################
# This one is training and testing on the Sighthill data with mis-detection and frame drop for simualated
# mis-detection. The combined mis detection and Particle Filter based trajectory generation and finally RMSE 
# comparison with /0/10/20/30% mis-detection ("frameDropRat"e) is done in this version. Withut mis-detection
# and normal prediction is done in the previous version, V19. 
###############################################################################################################
...
# do file comp for identify changes for visualization
# extra batch norm in position.. not converging...
###############################################################################################################
...
trainStr = 'Train'
testStr = 'Test'
trainOrTest = testStr   #   do not change just for frame drop.. the model is old 
...
# validationVehicleList = [58,47,63,57,20,36,11,25,52,6,25,38,111,32,122,123,112,50,126,86]     # validation new list for train  58,47,66,57
# validationVehicleList = [58,47,66,57,32,33,92,111,112,109,25,20,126,50]  # ,20,36,11,25,52,6,25,38,111,32,122,123,112,50,126,86]     # validation new list for test removed: 63   added: 66
# validationVehicleList = [83,84,107,109]
...
# validationVehicleList = [4,57,115,124,157,59,94,158,98]   # Downward turns only..
...
validationVehicleList = [83,107,109,165]    # For the true maneuver and traj
...
def TestModel(encoder_model,decoder_model):
    ...
    for eachTrainCar,eachTrainCarDropped in zip(carInfoList,carInfoListDropped):
        ...
        for idx in range(historyTemporal,currentVehicleLength-futureTemporal,1):
...
def __main__():
    ...
    if(trainOrTest == trainStr):
        ...
        # Save the model
        encoder_model.save_weights('/home/saptarshi/PythonCode/Junction/NavtechModels/encoderV19.h5')
        decoder_model.save_weights('/home/saptarshi/PythonCode/Junction/NavtechModels/decoderV19.h5')


    elif(trainOrTest == testStr):
        print('Starting testing !!!')

        encoder_model.load_weights('/home/saptarshi/PythonCode/Junction/NavtechModels/encoderV19.h5')
        decoder_model.load_weights('/home/saptarshi/PythonCode/Junction/NavtechModels/decoderV19.h5')
